...
for (const skillName of ['기본 공격', '연속 베기']) test(`${skillName} draws its real hits on the battlefield`, async ({ page }, testInfo) => {
    ...
    // Keep a real enemy alive long enough to observe both contacts after weapon damage buffs.
    await page.evaluate(skillName => {
        ...
        game.gridPlayer={gx:3,gy:4};game.moveTimer=0;
        game.enemies=[Object.assign(createEnemy(getZone(0),{at:0,count:1},0),
            {gx:4,gy:4,hp:100000,maxHp:100000})];
        switchTab('tab-battle');
    }, skillName);
    const transparency = await page.evaluate(skillName => {
        const image = battleAssets.images[skillName === '기본 공격' ? 'skillFxBasicSlash' : 'skillFxWorldTree'];
    ...
    expect(transparency.transparent).toBeGreaterThan(skillName === '기본 공격' ? 0.75 : 0.25);
        ...
        function observe() {
            ...
            for (const effect of battleVisualState.skillEffects) {
                ...
                const hit = battleFx.find(fx => fx.type === 'hit' && fx.skillName === skillName
                    && fx.damageTextGroupId === effect.vfxGroupId && !captured[fx.repeatIndex]
                    && (skillName === '연속 베기' || fx.repeatIndex === effect.repeatIndex));
