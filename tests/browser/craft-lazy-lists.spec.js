...
test('craft target libraries render only when open and refresh after inventory changes',async({page},info)=>{
    ...
    await page.evaluate(()=>{
        ...
        for(const id of ['ui-fossil-material-actions','ui-chaos-infuser-panel'])new MutationObserver(rows=>hiddenCraftChanges+=rows.length).observe(document.getElementById(id),{childList:true,subtree:true});
    ...
    await (await currencyUse(page,'fossilBound')).click();
    await expect.poll(()=>page.evaluate(()=>game.currencies.fossilBound)).toBe(0);
    await page.locator('#cl-materials > summary').click();
    await page.locator('#ui-fossil-material-actions').getByRole('button',{name:/기본 화석 정제/}).click();
    await page.locator('#game-dialog-overlay').getByRole('spinbutton').fill('1');
