...
function generateOrganicTree() {
    ...
    function buildSpecializedCluster(anchor, theme, sectorIndex, clusterIndex) {
        if (!anchor) return;
        ...
        let laneSign = anchor.lane !== undefined && anchor.lane !== 0 ? Math.sign(anchor.lane) : (clusterIndex % 2 === 0 ? 1 : -1);
        let baseAngle = Math.atan2(anchor.y, anchor.x);
        let branchAngle = baseAngle + laneSign * (0.33 + (clusterIndex % 3) * 0.025);
        let chainLength = clusterIndex % 2 === 0 ? 5 : 4;
        let prev = anchor;
        for (let i = 1; i <= chainLength; i++) {
            let isEnd = i === chainLength;
            let step = 76 + i * 70;
            let curve = laneSign * Math.sin(i * 0.85) * 24;
            let x = (anchor.x / PASSIVE_WORLD_SCALE) + Math.cos(branchAngle) * (step / PASSIVE_WORLD_SCALE) - Math.sin(branchAngle) * (curve / PASSIVE_WORLD_SCALE);
            let y = (anchor.y / PASSIVE_WORLD_SCALE) + Math.sin(branchAngle) * (step / PASSIVE_WORLD_SCALE) + Math.cos(branchAngle) * (curve / PASSIVE_WORLD_SCALE);
            let tier = isEnd ? 3 : (i >= chainLength - 1 ? 2 : 1);
            let kind = isEnd ? 'keystone' : (i >= chainLength - 1 ? 'major' : 'node');
            let node = addNode(x, y, tier, themeSpec.stat, { sector: theme, kind: kind, depth: (anchor.depth || 0) + i, lane: anchor.lane });
            if (!node) return;
            node.clusterId = `${theme}_${sectorIndex}_${clusterIndex}`;
    ...
    for (let s = 0; s < sectorCount; s++) {
        ...
        clusterAnchors.forEach((spec, index) => {
            ...
            buildSpecializedCluster(anchor, theme, s, index);
        });
    }
