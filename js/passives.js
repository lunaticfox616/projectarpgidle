...
function initBattleAssets() {
    ...
    const heroManifest = Object.fromEntries(Object.values(HERO_SELECTION_DEFS).map(def => [def.strips.idle, def.spriteSrc]));
    const manifest = {
        ...heroManifest,
...
function buildBattleAssetAtlas() {
    ...
    function buildHeroFrameSetFromStripKeys(stripKeys, heroId) {
        ...
        let frameCount = (HERO_SELECTION_DEFS[heroId] || HERO_SELECTION_DEFS.hero1).spriteFrameCount;
        let idleFrames = normalizeFrameSetBasisHeight(buildStripFramesFromImage(battleAssets.images[stripKeys.idle], 210, frameCount));
        let walkFrames = normalizeFrameSetBasisHeight(buildStripFramesFromImage(battleAssets.images[stripKeys.walk], 210, frameCount));
        let attackFrames = normalizeFrameSetBasisHeight(buildStripFramesFromImage(battleAssets.images[stripKeys.attack], 220, frameCount));
        let hurtFrames = normalizeFrameSetBasisHeight(buildStripFramesFromImage(battleAssets.images[stripKeys.hurt], 200, frameCount));
        let downFrames = normalizeFrameSetBasisHeight(buildStripFramesFromImage(battleAssets.images[stripKeys.death], 200, frameCount));
    ...
    function resolveHeroImageForFrameSet() {
        ...
        return battleAssets.images[strips.idle]
            ...
            || battleAssets.images.hero1Sprite
            || legacyHeroImage;
    }
