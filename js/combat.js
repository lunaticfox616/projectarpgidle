...
// Stable calculator entry for isolated requirement views, independent of public instrumentation wrappers.
const equipmentStatCalculator = getPlayerStats;
...
let trialHazardRuntime = { nextAt: 0, active: null };
let skillGemCombatRuntime = null;

// Legacy runtime state that must follow the game snapshot during offline calculation.
...
function resetCombatChannelRuntime() {
    if (skillGemCombatRuntime) skillGemCombatRuntime.channel = null;
    ...
    if (game) game.passiveChannelPath = null;
}
...
function updateCombatChannelRuntime(now) {
    ...
    if (timestamp >= combatChannelRuntime.endAt) {
        ...
        combatChannelRuntime = { ...combatChannelRuntime, id: 0 };
    }
    return combatChannelRuntime.id > 0;
}

/** A double strike shares the current cycle; continued cycles retain their absolute deadline.
 * @param {string} skillName @param {number} motionMs @param {number} cycleMs
 * @param {{repeatChannelCast:boolean,channelContinuation:boolean}} context
 * @returns {{id:number,skillName:string,startAt?:number,endAt:number}}
 */
function beginCombatChannel(skillName, motionMs, cycleMs, context) {
    if (context.repeatChannelCast && combatChannelRuntime.id) return combatChannelRuntime;
    let startAt = getCombatTime() + motionMs;
    if (context.channelContinuation) startAt = combatChannelRuntime.endAt || getCombatTime();
    combatChannelRuntime = {
        id: nextCombatChannelId++, skillName, startAt, endAt: startAt + cycleMs
    };
    return combatChannelRuntime;
}

function getCombatChannelCycleMs(attackSpeed) {
    let attacksPerSecond = Math.max(0.01, Number(attackSpeed) || 1);
    return Math.max(1, Math.min(5000, 1000 / attacksPerSecond));
}
...
function consumeCombatChannelContinuation(skillName) {
    let continues = combatChannelResumeSkillName === String(skillName || '') || (!!combatChannelRuntime.id && combatChannelRuntime.skillName === skillName);
    combatChannelResumeSkillName = '';
    return continues;
}
...
/** @param {number} damage @param {number} chancePct @param {number} damagePct @param {boolean} overflow */
function getAverageCriticalHitDamage(damage, chancePct, damagePct, overflow) {
    let chance = Math.max(0, chancePct / 100);
    if (!overflow) chance = Math.min(1, chance);
    return damage * (1 + (Math.max(1, damagePct / 100) - 1) * chance);
}
...
function resolveEntropyEvasion(defender, evadeChance, now) {
    ...
    let currentTime = Number.isFinite(now) ? now : getCombatTime();
...
function getPlayerHitLeechPercent(pStats) {
    const tags = pStats.sSkill?.tags || [];
    if (tags.includes('spell')) return Math.max(0, pStats.spellLeech || 0);
    if (pStats.passiveKeystoneFlags?.soulSanctuary) return 0;
    return tags.includes('attack') ? Math.max(0, pStats.leech || 0) : 0;
}
...
function applyPlayerHitLeech(pStats, hitDamage, chaosDamage) {
    const target = getPlayerHitLeechTarget(pStats);
    const attackAllowed = pStats.sSkill.tags.includes('attack') && !pStats.passiveKeystoneFlags?.soulSanctuary;
    const chaosPct = attackAllowed ? pStats.uniqueChaosDamageInstantLeechPct : 0;
    let recovered = applyInstantPlayerLeech(chaosDamage * chaosPct / 100, pStats, target);
    const amount = hitDamage * getPlayerHitLeechPercent(pStats) / 100;
    if (!(amount > 0)) return recovered;
    recovered += applyInstantPlayerLeech(amount * pStats.uniqueInstantLeechPct / 100, pStats, target);
    if (pStats.sSkill.instantLeech) recovered += applyInstantPlayerLeech(amount, pStats, target);
    else addPlayerLeechInstance(amount, pStats, target);
    return recovered;
}
...
function refreshRealmDeathWard(pStats) {
    ...
    if ((state.amount || 0) <= 0 && getCombatTime() >= (state.readyAt || 0)) {
...
function absorbDamageWithRealmDeathWard(amount, pStats) {
    ...
    if (state.amount <= 0) {
        ...
        state.readyAt = getCombatTime() + cooldownMs;
    }
...
function absorbDamageWithTalentStoneShield(amount) {
    ...
    if ((runtime.stoneShieldExpiresAt || 0) <= getCombatTime()) {
...
function getElementalistOverloadStacks() {
    return Math.max(0, Math.min(ELEMENTALIST_OVERLOAD_STACK_MAX,
        Math.floor(game.elementalistOverloadStacks || 0)));
}
...
function recordElementalistOverloadAttack(isCrit) {
    ...
    return game.elementalistOverloadStacks;
}
...
function isDualWielding() {
    ...
    let mainWeapon = equipment && equipment['무기'];
    let shieldWeapon = equipment && equipment['방패'];
...
function getSummonRuntimeCap(pStats) {
    if (pStats && pStats.passiveKeystoneFlags && pStats.passiveKeystoneFlags.soleMinion) return 1;
...
function buildActiveSummonRuntimeDefs(pStats) {
    ...
    if (pStats && pStats.passiveKeystoneFlags && pStats.passiveKeystoneFlags.soleMinion) {
        let guardDefs = defs.filter(row => getSummonProfile(row.name).role === 'guard');
        let attackDef = defs.find(row => getSummonProfile(row.name).role === 'attack');
        defs = [...(attackDef ? [attackDef] : []), ...guardDefs];
        return defs.map((row, idx) => ({ ...row, slotIdx: idx, duplicateIndex: row.duplicateIndex || 0 }));
    }
...
function getSummonMaxHp(profile, gemLevel, pStats) {
    ...
    if (flags.proxyCovenant && profile.role === 'guard') hpMul *= 2;
    if (flags.soleMinion && profile.role === 'attack') hpMul *= 1 + Math.max(0, pStats.passiveSoleMinionLostCap || 0) * 0.5;
...
function buildSummonRuntimeStats(row, pStats, now, gemLv = getSummonGemLevel(row.name, row.source, pStats)) {
    ...
    return {
        ...
        respawnMs: Math.max(4000, Math.floor((profile.respawnMs || (isGuard ? 8000 : 4000))
            * (((isGuard && pStats.passiveKeystoneFlags && pStats.passiveKeystoneFlags.proxyCovenant)
                || (!isGuard && pStats.passiveKeystoneFlags && pStats.passiveKeystoneFlags.soleMinion)) ? 2 : 1))),
...
function getPassiveWisdomLeapDamageMultiplier(pStats, element) {
    let selectedElement = pStats && pStats.passiveWisdomElement;
    if (!selectedElement) return 1;
    let damageElement = String(element || '').replace('lightning', 'light');
    return damageElement === selectedElement ? 1.2 : 0;
}

function getLimitedSummonFinalDamageMultiplier(pStats, summon) {
    // 소환수도 플레이어의 일반 최종 피해 배율과 지혜의 도약 속성 선택을 적용받습니다.
    let finalMultiplier = Number(pStats && pStats.finalDamageMultiplier);
    if (!Number.isFinite(finalMultiplier)) finalMultiplier = 1;
    return Math.max(0, finalMultiplier) * getPassiveWisdomLeapDamageMultiplier(pStats, summon && summon.ele);
}
...
function restoreAndRecallSummons(pStats) {
    ...
    (game.summons || []).forEach(summon => {
        ...
        summon.respawnAt = 0;
        summon.nextAttackAt = getCombatTime() + 300;
        delete summon.gx;
        delete summon.gy;
    });
    applyGrandlordGhostState();
...
function getSummonPierceTargets(target, enemies) {
    ...
    return enemies.filter(enemy => enemy && enemy !== target && enemy.hp > 0 && hasGridCell(enemy)
        && getGridUnitDistance(target, enemy) <= 1);
}
...
function coreLoop(nowMs) {
    ...
    activeConditionEffects.forEach(({ delta }) => {
        ...
        if (delta.cleanseIgnite || delta.cleanseChill || delta.cleanseFreeze || delta.cleanseShock || delta.cleanseBleed) {
            game.playerAilments = (game.playerAilments || []).filter(ail => {
                if (!ail) return false;
                let cleansed = (delta.cleanseIgnite && ail.type === 'ignite') || (delta.cleanseChill && ail.type === 'chill')
                    || (delta.cleanseFreeze && ail.type === 'freeze') || (delta.cleanseShock && ail.type === 'shock')
                    || (delta.cleanseBleed && ail.type === 'bleed');
                if (cleansed && typeof recordPassiveCycleAilmentEnd === 'function') {
                    recordPassiveCycleAilmentEnd(ail.type, pStats.cycle, getCombatTime());
                }
                if (cleansed) return false;
                return true;
            });
        }
    });
    ...
    if ((pStats.energyShield || 0) > 0 && game.playerEnergyShield < energyShieldRecoveryCap) {
        ...
        if ((game.crusaderEsRegenUntil || 0) > getCombatTime()) {
            let regenPerSec = (pStats.energyShield || 0) * 0.25;
            game.playerEnergyShield = Math.min(energyShieldRecoveryCap, game.playerEnergyShield + regenPerSec * 0.1);
        }
    ...
    if ((game.season || 1) >= 9 && zoneNow && zoneNow.type === 'abyss') {
        ...
        if (v.active) {
            ...
            if (spawnVoidBreachReinforcement(zoneNow, v) && game.settings.showSpawnLog !== false) {
                addLog(`🕳️ 균열 출현 ${v.spawnedCount}/${v.totalToSpawn}`, 'attack-monster', { noToast: true });
            }
...
function getPendingSkillImpactTargets(row) {
    ...
    let used = new Set();
    let maxTargets = Math.max(1, row.options?.targetLimit || (row.targetEntries || []).length);
    return getPendingSkillCollisionCells(row).map(cell => {
        let enemy = (game.enemies || []).find(candidate => candidate && candidate.hp > 0
            && getGridUnitCells(candidate).some(occupied => occupied.gx === cell.gx && occupied.gy === cell.gy) && !used.has(candidate.id));
...
/** Visual anchors are independent of integer collision cells and contact times. */
function getSkillTravelVisualEndpoints(row) {
    if (row.delivery !== 'projectileTarget' || row.travelPath || row.patternKind === 'boomerang') {
        return { sourceCell: row.sourceCell, targetCells: row.targetCells };
    }
    const enemies = game.enemies || [];
    const source = enemies.find(enemy => enemy.id === row.options.chainFromEnemyId);
    const targetCells = row.targetCells.map((cell, index) => {
        const target = enemies.find(enemy => enemy.id === row.targetEntries[index]?.enemyId);
        return hasGridCell(target) ? getGridUnitCenter(target) : cell;
    });
...
function addPendingSkillTravelFx(row, attackContext, now) {
    ...
    const visual = getSkillTravelVisualEndpoints(row);
    let visualTargetCells = visual.targetCells;
 ...
 */
function buildLinearSkillVisualRow(rows) {
    const first = rows[0], last = rows[rows.length - 1];
    const source = first.sourceCell;
    ...
    if (remaining > 0) {
        const skill = first.pStats.sSkill;
        const tailMs = first.patternKind === 'moving'
            ? remaining * Math.max(80, Number(skill.combatPattern.intervalMs) || 160)
            : getCombatTravelMs(tip, end, skill) - getCombatTravelMs(tip, tip, skill);
...
function addPendingSkillTravelFxRows(rows, attackContext, now) {
    ...
    if (attackContext.skillName === '빙결 침식') {
        const last = queuedRows[queuedRows.length - 1];
        addPendingSkillTravelFx({
            ...firstRow, options: { ...firstRow.options, attackFootprint: last.options.attackFootprint },
            fieldDurationMs: Math.max(last.at - now + 160, firstRow.at - now + 1000 / Math.max(0.01, firstRow.pStats.aspd))
        }, attackContext, now);
        return;
    }
    ...
    addPendingSkillTravelFx(buildLinearSkillVisualRow(queuedRows), attackContext, now);
}

/** Schedules one attack-speed cycle. Double strikes share its time window.
 * Channel continuity follows absolute stage times, independently of visual/tick cadence.
 * @param {ReturnType<typeof getPlayerStats>} pStats
 * @param {ReturnType<typeof buildSkillHitSequence>} stages
 * @param {{skillName:string,forcedCrit:boolean,forcedElement:string,stageCount:number,talentAttackMultiplier:number,attackDamageMultiplier:number,channelContinuation:boolean,repeatChannelCast:boolean}} context
 */
function queuePlayerAttackSequence(pStats, stages, context) {
    if (pStats.sSkill.nativeCastId) return startSkillGemCombat(pStats, context);
    let tags = pStats.sSkill.tags || [], now = getCombatTime();
    let motionMs = tags.includes('slam') ? 460 : (tags.includes('projectile') ? 400 : 360);
    motionMs = Math.min(motionMs, Math.max(80, 800 / Math.max(0.01, pStats.aspd)));
    let isChannel = tags.includes('channeling');
    let cycleMs = isChannel ? getCombatChannelCycleMs(pStats.aspd) : 0;
    let timing = isChannel ? beginCombatChannel(context.skillName, motionMs, cycleMs, context) : { id: 0, startAt: now + motionMs };
    let delayMs = timing.startAt - now, channelId = timing.id;
    let duration = Math.max(80, delayMs + cycleMs);
    let damageTextGroupId = nextPlayerDamageTextGroupId++;
    if (nextPlayerDamageTextGroupId > 1000000000) nextPlayerDamageTextGroupId = 1;
    noteCombatTacticAttack(duration);
    ...
    (stages || []).forEach((stage, stageOffset) => {
        ...
        let row = {
            ...
            options: {
                ...
                stageRepeatOnce: !!stage.singleRepeat, skipGridControl: stage.skipGridControl,
                ...
                passiveKarmaContext: attackContext.passiveKarmaContext || null
            }
        };
...
const getPassiveSpecialStatBreakdowns = function(rules) {
    const mystique = Math.max(0, Number(rules.mystique && rules.mystique.damagePct) || 0);
    const cycle = Math.max(0, Number(rules.cycle) || 0);
    const devotion = Math.max(0, Number(rules.devotion) || 0);
    const ailmentLabel = getAilmentDisplayLabel(rules.mystique && rules.mystique.ailment);
    const activeCycleLines = (rules.activeCycleBuffEffects || []).map(effect => {
        const stat = P_STATS[effect.stat] || {};
        const suffix = stat.isPct ? '%' : '';
        return `${getAilmentDisplayLabel(effect.source)} 종료 효과: ${getStatName(effect.stat)} +${formatValue(effect.stat, effect.val)}${suffix}`;
    });
    let revelationEffect = '헌신을 1 이상 확보하면 계시를 선택할 수 있습니다.';
    if (devotion > 0 && rules.revelation === 'combat') {
        revelationEffect = `주는 피해 ${rules.combatDamageMorePct}% 증폭`;
    ...
    return {
        ...
        cycle: {
            title: '순환',
            lines: ['적에게 적용한 상태이상이 종료되면 해당 유형에 따른 강화가 6초간 적용됩니다.',
                '순환 1당 대응 속성 피해 +2%와 유형별 보조 효과를 얻습니다.',
                ...(activeCycleLines.length > 0 ? activeCycleLines : ['현재 활성화된 순환 강화 없음']),
                rules.cycleAddedFireFromPhysicalPct > 0 ? `순환의 원석: 물리 피해의 ${rules.cycleAddedFireFromPhysicalPct}%를 화염 피해로 추가` : null
            ].filter(Boolean),
            final: formatValue('cycle', cycle)
        },
        revelation: {
            title: '계시',
            lines: [`계시 수치(헌신) ${formatValue('devotion', devotion)}`, revelationEffect],
            final: devotion > 0 ? rules.revelationLabel : '미해금'
        }
    };
};
...
function getPlayerStats(includeBreakdowns = !game.isBackgroundCalculation, attributesOnly = false, equipmentView = false) {
    ...
    equippedUniqueEffects.forEach(effect => {
        ...
        else if (effect.key === 'dsAndTargetAnyBonus') {
            addStatToBucket(reward, 'ds', Number(ep.ds !== undefined ? ep.ds : 8));
            addStatToBucket(reward, 'targetAny', Number(ep.target !== undefined ? ep.target : 1));
        }
    ...
    (heroDef && Array.isArray(heroDef.stats) ? heroDef.stats : []).forEach(row => {
    ...
    let authoredPassiveRules = typeof applyAuthoredPassiveStatRules === 'function'
        ...
        : { mystique: { ailment: 'bleed', damagePct: 0, potencyPct: 0 }, devotion: 0, cycle: 0,
            ...
            combatDamageMorePct: 0, guardTakenLessPct: 0, lifeBonusPct: 0,
            cycleAddedFireFromPhysicalPct: 0, fanaticismStacks: 0, activeCycleBuffEffects: [], flags: {} };
    ...
    if (attributesOnly) return { strength: totalStrength, dexterity: totalDexterity, intelligence: totalIntelligence };
    ...
    let gearExplicitTagged = getTaggedDamageBreakdown(gearExplicit, damageSkill);
    let passiveTagged = getTaggedDamageBreakdown(passive, damageSkill);
    let seasonTagged = getTaggedDamageBreakdown(season, damageSkill);
    let ascendTagged = getTaggedDamageBreakdown(ascend, damageSkill);
    let supportTagged = getTaggedDamageBreakdown(support, damageSkill);
    let rewardTagged = getTaggedDamageBreakdown(reward, damageSkill);
    let starTagged = getTaggedDamageBreakdown(starBlessing, damageSkill);
    ...
    const bleedChanceMultiplier = authoredPassiveRules.flags.bloodCradle ? 0.7 : 1;
    finalBleedChance *= bleedChanceMultiplier;
    ailmentCritChance.bleed *= bleedChanceMultiplier;
    ...
    let equippedShield = game.equipment && game.equipment['방패'];
    if (skill.shieldDamageBonusPct && equippedShield && equippedShield.slot === '방패') {
        finalBaseDmg = Math.floor(finalBaseDmg * (1 + Math.max(0, Number(skill.shieldDamageBonusPct) || 0) / 100));
    }
    ...
    if (authoredPassiveRules.fanaticismStacks > 0) {
        finalDamageMultiplier *= 1 + authoredPassiveRules.fanaticismStacks * 0.015;
    }
    ...
    let omniscientKeystone = typeof findAllocatedPassiveKeystone === 'function' && findAllocatedPassiveKeystone('사중합일');
    let omniscientActive = !!omniscientKeystone && totalStrength >= 20 && totalStrength === totalDexterity
        && totalDexterity === totalIntelligence && totalIntelligence === Math.max(0, passive.mystique);
    if (omniscientActive) finalEnergyShield += Math.floor((totalStrength + totalDexterity + totalIntelligence + passive.mystique) * 2);
    ...
    if (cosmosDeepSeaLeechCaps) {
        ...
        finalSpellLeech *= Math.max(0, 1 - Math.max(0, cosmosDeepSeaLeechCaps.rateLessPct || 20) / 100);
    ...
    genericTakenDamageMultiplier *= 1 - Math.max(0, authoredPassiveRules.guardTakenLessPct) / 100;
    let forbiddenManifestation = typeof findAllocatedPassiveKeystone === 'function' && findAllocatedPassiveKeystone('금단의 만상');
    if (forbiddenManifestation) {
        finalArmor = Math.floor(finalArmor * 0.4);
        finalEvasion = Math.floor(finalEvasion * 0.4);
        finalEnergyShield = Math.floor(finalEnergyShield * 0.4);
        finalDeflectChance = Math.min(25, finalDeflectChance);
    }
    if (authoredPassiveRules.fanaticismStacks > 0) {
        let defenseMultiplier = Math.max(0, 1 - authoredPassiveRules.fanaticismStacks * 0.0075);
        finalArmor = Math.floor(finalArmor * defenseMultiplier);
        finalEvasion = Math.floor(finalEvasion * defenseMultiplier);
        finalEnergyShield = Math.floor(finalEnergyShield * defenseMultiplier);
        finalAspd = Math.min(12, finalAspd * (1 + authoredPassiveRules.fanaticismStacks * 0.005));
    }
    ...
    let ailmentPowerMultiplier = 1 + Math.max(0, sumStatAcrossBuckets('ailmentPotencyPct')) / 100;
    ...
    if (authoredPassiveRules.flags.fullEvasion) finalArmor = 0;
    ...
    if (authoredPassiveRules.flags.fullEvasion) evadeChance = Math.min(90, evadeChance + 15);
    ...
    let wisdomSkillDamageMultiplier = Math.max(0, Number(previewTalentConversion.mainPct) || 0)
        * getPassiveWisdomLeapDamageMultiplier(wisdomPreviewStats, previewSkillElement);
    let previewAddedDamagePct = { ...coreCubeAddedDamagePct };
    Object.entries(previewTalentConversion.added || {}).forEach(([element, value]) => {
        previewAddedDamagePct[element] = (Number(previewAddedDamagePct[element]) || 0) + (Number(value) || 0);
    });
    let wisdomAddedDamageMultiplier = Object.entries(previewAddedDamagePct).reduce((sum, [element, value]) => {
        let addedRatio = Math.max(0, Number(value) || 0) / 100;
        return sum + addedRatio * getPassiveWisdomLeapDamageMultiplier(wisdomPreviewStats, element);
    }, 0);
    let expectedAddedDamageMultiplier = 1 + coreCubeAddedDamageTotalPct / 100;
    let expectedWisdomDamageMultiplier = wisdomSkillDamageMultiplier + wisdomAddedDamageMultiplier;
    if (expectedAddedDamageMultiplier > 1) damageScales.coreCubeAddedDamageMultiplier = expectedAddedDamageMultiplier;
    if (wisdomLeapActive) damageScales.wisdomLeapDamageMultiplier = expectedWisdomDamageMultiplier;
    ...
    if (wisdomLeapActive) warriorPhysicalDpsMultiplier = previewSkillElement === 'phys' ? warriorPhysDamageMultiplier : 1;
    let dpsDamageMultiplier = instantDamageMultiplier * finalDamageMultiplier * warriorPhysicalDpsMultiplier
        * (previewSkillElement === 'chaos' ? chaosDamageMultiplier : 1);
    let finalDpsAdjusted = finalDps * avgRollMultiplier * expectedDoubleStrikeMultiplier * dpsDamageMultiplier * expectedWisdomDamageMultiplier;
    let flatElementAverage = Object.entries(flatElementHitDamage).reduce((sum, [element, value]) => sum + value
        * getPassiveWisdomLeapDamageMultiplier(wisdomPreviewStats, element) * (element === 'phys' ? warriorPhysDamageMultiplier : 1), 0);
    finalDpsAdjusted += flatElementAverage * (avgHit / Math.max(1, finalBaseDmg)) * finalAspd * avgRollMultiplier
        * expectedDoubleStrikeMultiplier * finalDamageMultiplier;
    ...
    if (isDotSkill) {
        ...
        let expectedDotSourceHit = avgHit * avgRollMultiplier * warriorPhysicalDpsMultiplier
            * (previewSkillElement === 'chaos' ? chaosDamageMultiplier : 1) * wisdomSkillDamageMultiplier;
    ...
    let finalBlockChanceCap = forbiddenManifestation ? 25 : Math.max(50, Math.min(75, 50 + Math.max(0, sumStatAcrossBuckets('blockChanceMax'))));
    ...
    if (authoredPassiveRules.flags.fullEvasion || authoredPassiveRules.flags.proxyCovenant) finalBlockChance = 0;
    ...
    let breakdowns = includeBreakdowns ? {
        ...
        spellLeech: {
            title: '주문 피해의 생명력 흡수',
            lines: [makeSourceLine('스킬', skill.spellLeech || 0, '%', value => `${formatValue('spellLeech', value)}%`),
                makeSourceLine('장비', gearBase.spellLeech + gearExplicit.spellLeech, '%', value => `${formatValue('spellLeech', value)}%`),
                makeSourceLine('기타 성장', passive.spellLeech + season.spellLeech + ascend.spellLeech + support.spellLeech + reward.spellLeech, '%', value => `${formatValue('spellLeech', value)}%`),
                '주문 타격 피해에만 적용됩니다. 지속 피해에는 적용되지 않습니다.'].filter(Boolean),
            final: `${formatValue('spellLeech', finalSpellLeech)}%`
        },
        ds: {
            title: '연속 타격',
            lines: [
    ...
    let enemy = {
        ...
        rawCritDmg: rawFinalCritDmg,
        regen: finalRegen,
        regenSuppress: finalRegenSuppress,
        leech: finalLeech,
        spellLeech: finalSpellLeech,
        ...
        mystique: Math.max(0, passive.mystique),
        devotion: authoredPassiveRules.devotion,
        cycle: authoredPassiveRules.cycle,
        passiveRevelation: authoredPassiveRules.revelation,
        passiveRevelationLabel: authoredPassiveRules.revelationLabel,
        passiveRevelationCombatDamageMorePct: authoredPassiveRules.combatDamageMorePct,
        passiveRevelationGuardTakenLessPct: authoredPassiveRules.guardTakenLessPct,
        passiveRevelationLifeBonusPct: authoredPassiveRules.lifeBonusPct,
        passiveWisdomElement: passiveWisdomElement,
        mystiqueAilmentType: authoredPassiveRules.mystique.ailment,
        mystiqueAilmentDamagePct: authoredPassiveRules.mystique.damagePct,
        mystiqueAilmentPotencyPct: authoredPassiveRules.mystique.potencyPct,
        passiveForbiddenManifestation: !!forbiddenManifestation,
        passiveAshura: typeof findAllocatedPassiveKeystone === 'function' && !!findAllocatedPassiveKeystone('아슈라'),
        passiveCycleAddedFireFromPhysicalPct: authoredPassiveRules.cycleAddedFireFromPhysicalPct,
        passiveFanaticismStacks: authoredPassiveRules.fanaticismStacks,
        passiveKeystoneFlags: authoredPassiveRules.flags,
        passiveOmniscientTrueDamagePct: omniscientActive ? Math.min(25, totalStrength * 0.5) : 0,
        passiveAlwaysHit: !!barbarismKeystone,
        chillEffectBonusPct: finalChillEffectBonusPct,
        ...
        projectileExtraShots: Math.max(0, totalProjectileExtraShots),
    ...
    enemy.passiveSoleMinionLostCap = authoredPassiveRules.flags.soleMinion
...
function getPlayerTacticalMovePlan(pStats, target) {
    let tactics = resolveConditionalCombatTactics(normalizeCombatTacticsSettings(game.settings), pStats, game, getCombatTime());
    let profile = getSkillGridProfile(game.activeSkill, pStats.sSkill);
    let distance = getGridUnitDistance(game.gridPlayer, target);
    if (pStats.passiveKeystoneFlags && pStats.passiveKeystoneFlags.farshot && distance < 3) {
        return { direction: 'away', range: Math.max(3, Number(profile.range) || 1) };
    }
...
function movePlayerForCharge(target, maxCells) {
    ...
    for (let step = 0; step < maxCells; step++) {
        if (getGridUnitDistance(game.gridPlayer, target) <= 1) break;
        if (!gridStepToward(game.gridPlayer, target.gx, target.gy, getGridBlockedCells(game.gridPlayer))) break;
        moved = true;
    }
    return moved ? origin : null;
}

function movePlayerForBlink(target) {
    if (getGridUnitDistance(game.gridPlayer, target) <= 1) return null;
    let destination = findFreeGridCell(getGridBlockedCells(game.gridPlayer), target);
...
function applySkillMobilityBeforeAttack(skill, target, pStats) {
    let pattern = skill && skill.mobilityPattern;
    if (!pattern || !target || !hasGridCell(target) || !hasGridCell(game.gridPlayer)) return false;
    let maxCells = Math.max(1, Math.min(6, Math.floor(Number(pattern.maxCells) || 1)));
    let origin = pattern.kind === 'blink' ? movePlayerForBlink(target) : movePlayerForCharge(target, maxCells);
    if (!origin) return false;
    let moveSpeed = Number.isFinite(pStats.moveSpeed) && pStats.moveSpeed > 0 ? pStats.moveSpeed : 100;
    let delayMs = getTacticalMoveAttackDelayMs(moveSpeed);
    combatTacticsRuntime.attackDelayUntil = Math.max(combatTacticsRuntime.attackDelayUntil, getCombatTime() + delayMs);
    addBattleFx('playerMobility', { skillName: game.activeSkill, fromCell: origin, toCell: copyCombatGridCell(game.gridPlayer), duration: pattern.kind === 'blink' ? 180 : 280 });
    return true;
}
...
function updatePlayerGridEngagement(pStats, options) {
    ...
    if (targets.length > 0) {
        ...
        let plan = !config.holdPosition && game.combatTacticsUnlocked && !stalled
            ? getPlayerTacticalMovePlan(pStats, targets[0].enemy)
            : null;
...
function rollEnemyTrait(zone, isElite, isBoss, seed) {
    ...
    if (zone && zone.type === 'trial' && zone.id === 'trial_4') {
        list = list.map(trait => trait.id === 'bloodless'
            ? { ...trait, id: 'leechResist_trial4', name: '흡혈저항', leechEffMul: 0.45 }
            : trait);
    }
    if ((game.season || 1) >= 10 && zone && zone.type === 'abyss' && zone.ele === 'chaos') {
        list.unshift({ id: 'veryFast_loop10', name: '매우 빠름', outlineColor: '#82d8de', attackSpeedVarMul: 1.34, expMul: 1.10, dropMul: 1.08 });
    }
...
function applyHolyMistMitigation(element,enemy,resistance) {
    return element==='fire' && enemy?.holyMistUntil>getCombatTime() ? 100-(100-resistance)*1.2 : resistance;
}
...
function applyCosmosBossSpecialDebuff(enemy, type) {
    ...
    row.value = Math.max(Number(row.value || 0), spec.value);
    row.durationMs = Math.floor(spec.duration * 1000);
    row.expiresAt = Math.max(Number(row.expiresAt || 0), getCombatTime() + row.durationMs);
}
...
function applyCosmosPlayerDebuffsToStats(pStats) {
    ...
    game.cosmosPlayerDebuffs.forEach(d => {
        ...
        if (d.type === 'cosmos_regen_down') {
            ...
            pStats.spellLeech = Math.max(0, (pStats.spellLeech || 0) * (1 - value / 100));
...
function createEnemy(zone, marker, groupIndex) {
    const level = levelProgression.monsterLevel(zone, marker);
    ...
    let enemy = {
        ...
        monsterArchetype: wispVisual ? 'wisp' : null,
        ...
        recentHitsTaken: 0,
        recentHitsTimer: 0,
        patternMode: (cosmosMods && cosmosMods.patternMode) || (isBoss
            ? (rndChoice(getBossPatternModesForLoop(game.season || 1)) || null)
            : null),
        ...
        traitOutlineColor: isElite && trait ? trait.outlineColor || '#e2b94f' : null,
        leechEffMul: trait && Number.isFinite(trait.leechEffMul) ? Math.max(0, trait.leechEffMul) : 1,
        expMul: (trait && Number.isFinite(trait.expMul) ? Math.max(1, trait.expMul) : 1) * (cosmosExclusiveTrait && Number.isFinite(cosmosExclusiveTrait.expMul) ? Math.max(1, cosmosExclusiveTrait.expMul) : 1) * (zone.type === 'beyondBoundary' ? Math.max(1, Number(zone.boundaryRewardMul) || 1) : 1),
        dropMul: (trait && Number.isFinite(trait.dropMul) ? Math.max(1, trait.dropMul) : 1) * (cosmosExclusiveTrait && Number.isFinite(cosmosExclusiveTrait.dropMul) ? Math.max(1, cosmosExclusiveTrait.dropMul) : 1) * getTierDropMulWithCaps(zone.tier, zone) * (zone.type === 'beyondBoundary' ? Math.max(1, Number(zone.boundaryRewardMul) || 1) : 1),
        isSky: isSky
    };
    if (zone.type === 'beyondBoundary') {
        enemy.dr = Math.min(90, enemy.dr + Math.max(0, Number(zone.boundaryDrBonus) || 0));
        enemy.penetration += Math.max(0, Number(zone.boundaryPenetrationBonus) || 0);
    }
...
function getZoneEncounterProfile(zone) {
    ...
    if (zone.type === 'meteor') return { markerCount: 2, minPack: 2, maxPack: 3, eliteChance: 0.5, bossAdds: 0, label: '운석' };
...
function resolveMapEstimateContentScale(zone) {
    ...
    if (benchmark) {
        return {
            hp: benchmark.hp * (zone.type === 'beyondBoundary' ? Math.max(1, Number(zone.boundaryHpMul) || 1) : 1),
            damage: benchmark.damage * (zone.type === 'beyondBoundary' ? Math.max(1, Number(zone.boundaryDamageMul) || 1) : 1),
            clearTimeSec: benchmark.clearTimeSec
        };
    }
    ...
    if (zone.type === 'beehive') scale.clearTimeSec = 30;
    if (zone.type === 'colony') {
        scale.hp = 3.4;
        scale.damage = 1.45;
    }
...
function getMapEstimateThreatProfile(zone, bossMods, baseHit, seasonDepth, tier) {
    ...
    return {
        ...
        resistancePressure: ((game.season || 1) >= 4 ? 14 : 0)
            ...
            + (zone.type === 'beyondBoundary' ? Math.max(0, Number(zone.boundaryPenetrationBonus) || 0) : 0)
    };
}
...
function generateEncounterPlan(zone) {
    ...
    if (zone.type === 'meteor') return [{ at: 36, count: 3 }, { at: 76, count: 2, elite: true }, { at: 100, count: 1, boss: true }];
...
function getTrialHazardProfile(zone) {
    let source = zone && zone.trialHazard ? zone.trialHazard : {};
    return {
        pattern: ['line', 'doubleLine', 'pool', 'cross', 'block'].includes(source.pattern) ? source.pattern : 'line',
        warningMs: Math.max(800, Math.min(3000, Number(source.warningMs) || 1700)),
        intervalMs: Math.max(2400, Math.min(12000, Number(source.intervalMs) || 6000)),
        initialDelayMs: Math.max(1000, Math.min(6000, Number(source.initialDelayMs) || 2200)),
        avoidThresholdMul: Math.max(0.5, Math.min(1.5, Number(source.avoidThresholdMul) || 1))
    };
}
...
function primeTrialHazardTimer(zone) {
    ...
    game.trialHazardIndex = 0;
    trialHazardRuntime = {
        nextAt: getCombatTime() + getTrialHazardProfile(zone).initialDelayMs,
        active: null
    };
}
...
function unlockLoopBloomSpecialization(heroId, classKey, classLabel, firstEverBloomOfClass) {
    if (!classKey || classKey === 'none' || game.bloomLoopSpecGranted === classKey) return false;
    game.bloomLoopSpecGranted = classKey;
    game.bloomedClassThisLoop = classKey;
    game.bloomedTalentThisLoop = heroId;
...
function awardTalentBloomCard(comboKey, heroLabel, classLabel) {
    if (typeof recordTalentBloomCard !== 'function') return;
...
// 재능 개화 시련 클리어 처리. 카드 조합은 입장 시 고르고, 5차 특화 포인트는 루프당 한 번만 지급한다.
function handleTalentBloomClear(zone) {
    ...
    let heroId = resolveTalentBloomHeroId(), classKey = game.ascendClass || 'none';
    ...
    unlockLoopBloomSpecialization(heroId, classKey, classLabel, firstEverBloomOfClass);
    awardTalentBloomCard(comboKey, heroLabel, classLabel);
...
function getSkillConditionalDamageMultiplier(skill, enemy, onCue) {
    ...
    if (active && ailments.some(ail => ail && ail.type === active.type && (ail.time || 0) > 0)) {
        let pct = Math.max(0, Number(active.pct) || 0);
    ...
    (skill.consumeAilmentDamageMore || []).forEach(config => {
        ...
        if (index < 0) return;
        let pct = Math.max(0, Number(config.pct) || 0);
        multiplier *= 1 + pct / 100;
...
/** The cast snapshot owns a targeted pull destination; legacy player pulls follow the player. */
function getSkillGridPullGeometry(skill, options) {
    let cells = Math.max(0, Math.min(3, Math.floor(Number(skill?.pullTowardImpactCells || skill?.pullTowardPlayerCells) || 0)));
    let center = skill?.pullTowardImpactCells ? options.attackFootprint?.center : game.gridPlayer;
    return { cells, center };
}

function applySkillGridControlOnHit(enemy, skill, options = {}) {
    let { cells, center } = getSkillGridPullGeometry(skill, options);
    if (cells <= 0 || options.skipGridControl || !enemy || enemy.hp <= 0 || !center || !hasGridCell(enemy)) return false;
    let moved = false;
    for (let step = 0; step < cells; step++) {
        if (!gridStepToward(enemy, center.gx, center.gy, getGridBlockedCells(enemy))) break;
        moved = true;
    }
...
function spreadSkillAilmentOnHit(source, skill, pStats) {
    ...
    targets.slice(0, Math.max(1, Math.floor(config.targets || 1))).forEach(target => {
        mergeEnemyAilment(target, copy, pStats);
        addBattleFx('statusText', {
            enemyId: target.id,
            text: `${getAilmentDisplayLabel(config.type)} 전파`,
            color: getElementColor(skill.ele),
            duration: 650,
            dedupeKey: `ailment-spread:${source.id}:${target.id}`,
            dedupeWindowMs: 650
        });
    });
}
...
function addConfirmedSkillEffect(kind, visual, target, element) {
    ...
    addBattleFx('combatTravel', {owner:'player',delivery:kind,skillName:visual.skillName,
        sourceCell:visual.sourceCell,targetCells:[getGridUnitCenter(target)],element,
...
function transferPassiveChaosErosionToTarget(enemy, pStats) {
    let flags = pStats && pStats.passiveKeystoneFlags;
    let pending = Math.max(0, Number(game.passiveChaosErosionTransfer) || 0);
    if (!flags || !flags.erosionLegacy || pending <= 0 || !enemy || enemy.hp <= 0) return false;
    enemy.chaosErosionShred = Math.min(pStats.chaosErosionCap || 40, (enemy.chaosErosionShred || 0) + pending);
    game.passiveChaosErosionTransfer = 0;
    return true;
}
...
function getPassiveBleedMultiplier(pStats) {
    return pStats?.passiveKeystoneFlags?.bloodCradle ? 0.7 : 1;
}

function getPlayerAilmentProcChance(pStats, type, isCrit, forcedChance) {
    const multiplier = type === 'bleed' ? getPassiveBleedMultiplier(pStats) : 1;
    const base = Number.isFinite(forcedChance) ? forcedChance * multiplier : getPlayerAilmentChance(pStats, type);
    return Math.max(0, Math.min(1, base + (isCrit ? 0.25 * multiplier : 0)));
}
...
function getShockedEnemyHitDamageIncreaseMultiplier(enemy, pStats) {
    let isShocked = enemy && Array.isArray(enemy.ailments)
        && enemy.ailments.some(ail => ail && ail.type === 'shock' && (ail.time || 0) > 0);
    let bonus = Math.max(0, Number(pStats && pStats.shockedEnemyHitDamagePct) || 0);
    if (!isShocked || bonus <= 0) return 1;
    let existingIncrease = Math.max(0, Number(pStats && pStats.damageIncreasePct) || 0);
    return (100 + existingIncrease + bonus) / (100 + existingIncrease);
}
...
function getEnemyDamageAilmentDps(ail, pStats, enemy) {
    ...
    dps = Math.floor(dps * (1 + Math.max(0, Number(pStats?.[damageStat]) || 0) / 100));
    if (ail && pStats && ail.type === pStats.mystiqueAilmentType) {
...
function applyEnemyAilmentFromHit(enemy, pStats, hitDamage, isCrit, options) {
    ...
    enemy.ailments = Array.isArray(enemy.ailments) ? enemy.ailments : [];
    function applyAilmentType(type, forceChance, manifestation) {
        let sourceType = type;
        let appliedType = typeof getTalentAilmentReplacement === 'function' ? getTalentAilmentReplacement(sourceType) : sourceType;
        let passiveFlags = (pStats && pStats.passiveKeystoneFlags) || {};
        if (passiveFlags.singleMystique && appliedType !== pStats.mystiqueAilmentType) return false;
        if (passiveFlags.blackDistill && ['ignite', 'chill', 'freeze', 'shock'].includes(appliedType)) return false;
        if (typeof canTalentCardApplyEnemyAilment === 'function' && !canTalentCardApplyEnemyAilment(appliedType)) return false;
        let tryProc = getPlayerAilmentProcChance(pStats, sourceType, isCrit, forceChance);
        if (Math.random() >= tryProc) return false;
        let resKey = 'ailRes' + sourceType.charAt(0).toUpperCase() + sourceType.slice(1);
        ...
        let mystiquePotency = sourceType === pStats.mystiqueAilmentType
            ? 1 + Math.max(0, Number(pStats.mystiqueAilmentPotencyPct) || 0) / 100 : 1;
        let manifestationPotency = manifestation ? 0.5 : 1;
        let effectiveHitRatio = hitRatio * mystiquePotency * manifestationPotency;
        if (sourceType === 'freeze' && Math.random() >= getFreezeApplyChanceFromHitRatio(effectiveHitRatio, enemy.maxHp || 1)) return false;
        type = appliedType;
        let damageAilment = isDamageAilmentType(type);
        let power = damageAilment
            ? 0.90
            : Math.max(0.05, Math.min(1.5, (hitPower + (hitRatio * 1.8)) * mystiquePotency * manifestationPotency));
        if (type === 'chill') {
            let chillEfficiency = 1 + Math.max(0, Number(pStats && pStats.chillEffectBonusPct) || 0) / 100;
        ...
        if (passiveFlags.taintedWarhead && ['ignite', 'poison', 'bleed'].includes(appliedType)) damageMorePct += 50;
        let payload = { type: type, time: dur, duration: dur, power: power, stacks: 1 };
        if (damageAilment) {
            payload.sourceHitDamage = Math.floor(ailmentPowerSourceDamage * mystiquePotency * manifestationPotency);
            payload.critDotBonusPct = critDotBonusPct;
            payload.ailmentDotScore = getDamageAilmentScore(payload.sourceHitDamage, critDotBonusPct,
                pStats && Number.isFinite(pStats.dotDamageScale) ? pStats.dotDamageScale : 1,
                pStats && Number.isFinite(pStats.dotCritBonusScale) ? pStats.dotCritBonusScale : 1) * (1 + damageMorePct / 100);
        ...
        if (row) {
            ...
            if (damageAilment) {
                ...
                if (incomingScore >= rowScore) {
                    row.sourceHitDamage = payload.sourceHitDamage;
                    row.critDotBonusPct = critDotBonusPct;
                    row.ailmentDotScore = incomingScore;
                    row.talentDamageMorePct = Math.max(0, Number(payload.talentDamageMorePct) || 0);
                    row.talentNitroAmplified = !!payload.talentNitroAmplified;
                }
            }
            if (damageAilment) {
    ...
    if (ele === 'cold' && !(typeof shouldTalentSkipColdFreeze === 'function' && shouldTalentSkipColdFreeze())) applyAilmentType('freeze');
    if (pStats.passiveForbiddenManifestation) {
        ['ignite', 'chill', 'shock', 'poison', 'bleed'].forEach(type => {
            if (type !== primaryType) applyAilmentType(type, undefined, true);
        });
    }
...
function startWoodsmanCurse() {
    ...
    game.woodsmanCurseDamageTakenStacks = 0;
    game.woodsmanCurseLastTickAt = getCombatTime();
...
function applyCosmosAstraStance(enemy) {
    ...
    let idx = Math.floor(getCombatTime() / COSMOS_ASTRA_STANCE_CYCLE_MS) % COSMOS_ASTRA_STANCES.length;
...
function startMoving(isTown) {
    ...
    game.moveTimer = time;
    if (returnDepartureCell) {
        addBattleFx('playerReturnDepart', {
            duration: Math.max(500, Math.round(time * 1000)),
            cell: returnDepartureCell
        });
    }
...
/** Append at most one reinforcement; capacity pauses the queue without consuming it.
 * @param {ReturnType<typeof getZone>} zone
 * @param {{pendingWave:boolean,spawnedCount:number,totalToSpawn:number,spawnTick:number}} rift Active run, mutated in place.
 * @returns {boolean} Whether an enemy was appended.
 */
function spawnVoidBreachReinforcement(zone, rift) {
    const alive = game.enemies.filter(e => e.hp > 0);
    if (!rift.pendingWave || rift.spawnedCount >= rift.totalToSpawn || alive.length >= 8
        || alive.filter(e => e.fromVoidRift).length >= 4) return false;
    rift.spawnTick++;
    if (rift.spawnTick < 4) return false;
    rift.spawnTick = 0;
    const index = rift.spawnedCount;
    const marker = { at: Math.max(5, Math.min(95, 10 + index * 8)), elite: Math.random() < 0.18, boss: false };
    const enemy = createEnemy(zone, marker, index + 1);
...
/** Spawn in the nearest unoccupied portal cell; a full board must never overlap its occupants. */
function spawnGrandBreachEnemy(zone, portal, blocked, elite, index) {
    const cell = findFreeGridCell(blocked, portal);
    if (!cell) return false;
    const enemy = createEnemy(zone, {elite,boss:false}, index);
    Object.assign(enemy, cell, {gridMoveTimer:0.25,attackTimer:0});
    blocked.add(gridCellKey(cell.gx,cell.gy));
    game.enemies.push(enemy);
    return true;
}

/** Append one bounded pack. Missed refill intervals are not replayed as a burst.
 * @param {ReturnType<typeof getZone>} zone
 * @param {{timeLeft:number,nextRefillAt?:number}} run Remaining seconds and next combat-clock refill time; mutated in place.
 * @param {number} now Combat clock milliseconds.
 */
function refillGrandBreachWave(zone, run, now) {
    if (run.timeLeft <= 0 || now < (run.nextRefillAt || 0)) return;
    const config = GRAND_BREACH_ENCOUNTER;
    const alive = game.enemies.filter(e => e.hp > 0);
    const stage = Number(run.timeLeft <= 25) + Number(run.timeLeft <= 12);
    const count = Math.min(alive.length === 0 ? config.openingPack : config.refillPack, config.capacities[stage] - alive.length);
    if (count <= 0) return;
    run.nextRefillAt = now + config.intervalMs;
    const blocked = getGridBlockedCells();
    let elites = alive.filter(e => e.isElite).length;
    for (let i = 0; i < count; i++) {
        const elite = elites < config.eliteLimit && i % 4 === 0;
        elites += Number(elite);
        const portal = config.portals[i % config.portals.length];
        if (!spawnGrandBreachEnemy(zone,portal,blocked,elite,i)) break;
    }
}
...
function tickGrandBreachRun(zone) {
    ...
    if (g.phase === 'survival') {
        ...
        refillGrandBreachWave(zone, g, now);
        if (g.timeLeft <= 0) {
            g.phase = 'boss';
...
function getEnemyExperienceReward(enemy, pStats) {
    let zone = getZone(game.currentZoneId);
    let abyssScale = getAbyssMonsterScales(zone);
    let abyssDepth = zone.type === 'abyss' ? Math.max(1, Math.floor(zone.depth || getAbyssDepthFromZoneId(zone.id) || 1)) : 0;
...
function applyGrandBreachMobTuning(zone, enemy) {
    ...
    enemy.damageMul = (enemy.damageMul || 1) * 1.25;
}
...
function applyVoidRiftMobTuning(enemy) {
    ...
    enemy.hp = enemy.maxHp;
    enemy.atkMul = (enemy.atkMul || 1) * 1.25;
    enemy.damageMul = (enemy.damageMul || 1) * 1.3;
...
// 선택한 직업의 권장 전투 방식에 맞는 하위권 화력 스킬 젬을 확정 지급한다.
...
function grantLoopStarterGemOnFirstKill() {
    ...
    let classDef = typeof PLAYER_CLASS_DEFS !== 'undefined' ? PLAYER_CLASS_DEFS[game.selectedClassId] : null;
    let heroId = classDef && HERO_SELECTION_DEFS[classDef.recommendedTalentHeroId]
        ? classDef.recommendedTalentHeroId : 'hero1';
    ...
    addLog(`스킬 젬 [${gemName}] 획득`, 'loot-rare');
}
...
function handleEnemyDeath(enemy, pStats) {
    ...
    if (pStats && pStats.uniqueLeechEfficiencyOnKill) {
        ...
        game.uniqueLeechEfficiencyUntil = getCombatTime() + durationMs;
    }
...
function grantGuaranteedTrialSkillGem() {
    ...
    if (available.length === 0) {
        let fallbackGain = grantGemResearchFragments(6, 'drop');
        if (fallbackGain > 0) addLog(`✨ 전직 시련 보상: 모든 공격 젬을 보유해 젬 잔향 +${fallbackGain}을 획득했습니다.`, 'loot-magic');
        return true;
    }
    let skill = rndChoice(available);
    game.skills.push(skill);
...
function getBeyondBoundaryCompletionRewardContext(result) {
    let focus = BEYOND_BOUNDARY_REWARD_FOCUS_DB.find(row => row.id === result.rewardFocusId)
        || BEYOND_BOUNDARY_REWARD_FOCUS_DB[0];
    // Pre-patch active runs retain their earned payout as equipment. New runs
    // reject a locked selection before payment instead of using this fallback.
    if (!getBeyondBoundaryRewardFocusStatus(focus.id).available) {
        focus = BEYOND_BOUNDARY_REWARD_FOCUS_DB[0];
    }
    let intensity = BEYOND_BOUNDARY_INTENSITY_DB.find(row => row.id === result.intensityId)
        || BEYOND_BOUNDARY_INTENSITY_DB[0];
    let tier = Math.max(1, Math.floor(Number(result.tier) || 1));
    let liveZone = getZone(BEYOND_BOUNDARY_ZONE_ID) || {};
    let zone = {
        ...liveZone, tier: getUnderworldTier(30) + Math.floor((tier - 1) / 2),
        boundaryTier: tier, name: `경계 너머 ${tier}단계 완료 보상`
    };
...
function grantBeyondBoundaryEquipmentRewards(context) {
    if (context.rewardChanged) addLog('선택한 보상이 잠겨 있어 이번 도전 보상을 장비로 지급합니다.', 'loot-rare');
    let count = rollBeyondBoundaryRewardCount(context.intensity.rewardMul);
    let names = [];
    for (let index = 0; index < count; index++) {
        let item = generateEquipmentDrop({ isBoss: true }, { minimumRarity: 'rare', zone: context.zone });
        if (!item || !addItemToInventory(item, { guaranteedKeep: true })) continue;
        names.push(item.name);
        addLog(`경계 완료 보상: [${item.name}]`, 'loot-rare', { item });
    }
    return `${names.length}개의 희귀 이상 장비`;
}

function grantBeyondBoundaryJewelRewards(context) {
    let count = rollBeyondBoundaryRewardCount(context.intensity.rewardMul);
    let stored = 0;
    for (let index = 0; index < count; index++) {
...
function grantBeyondBoundaryFocusedReward(result) {
    let context = getBeyondBoundaryCompletionRewardContext(result);
...
function getPassiveKeystoneHitMultiplier(pStats, target, hitIndex, targetIndex, skillName) {
    ...
    let multiplier = 1;
    if (flags.farshot && game.gridPlayer && Number.isFinite(target.gx)) {
        let distance = getGridUnitDistance(game.gridPlayer, target);
    ...
    if (flags.openingHunt) {
        ...
        target.passiveOpeningHuntConsumed = true;
    }
    if (!flags.channelPath) return multiplier;
    if (!(pStats.sSkill.tags || []).includes('channeling')) {
        game.passiveChannelPath = null;
        return multiplier * 0.8;
    }
    let state = game.passiveChannelPath || {};
    if (hitIndex === 0 && targetIndex === 0) {
        let sameTarget = state.skillName === skillName && String(state.targetId) === String(target.id);
        state = { skillName, targetId: target.id, stacks: sameTarget ? Math.min(6, (state.stacks || 0) + 1) : 1 };
        game.passiveChannelPath = state;
    }
    return multiplier * (1 + Math.max(0, state.stacks || 0) * 0.07);
}

/** Emit confirmed hit presentation, carrying the immutable cast-time attack cells. */
function addPlayerHitVisualFx(pStats, options, targetEnemy, hit) {
    ...
    addBattleFx('hit', {
        enemyId: targetEnemy.id, targetCell: hasGridCell(targetEnemy) ? getGridUnitCenter(targetEnemy) : null, channelId: options.channelId, color: getElementColor(hitElement), crit: hitCrit,
        ...
        stageKind, stageLabel, stageIndex: Math.max(0, Math.floor(Number(options.stageIndex) || 0)),
...
/** Combat owns damage and movement; gem controllers only return scheduled commands. */
function startSkillGemCombat(stats, options) {
    skillGemCombatRuntime ??= skillGemCasts.createState();
    const name=options.skillName || game.activeSkill;
    initializeSkillGemFacing();
    if (skillGemRuntimeChanged(name)) skillGemCombatRuntime=skillGemCasts.createState();
    if (name==='인과' && skillGemCombatRuntime.channel) return;
    const started=skillGemCasts.start(skillGemCombatRuntime,{id:stats.sSkill.nativeCastId,name,stats,
        source:game.gridPlayer,enemies:game.enemies,now:getCombatTime(),visuals:!game.isBackgroundCalculation,
        attackOptions:{forcedCrit:options.forcedCrit,forcedElement:options.forcedElement,
...
function shouldDeferNativeGemCast(stats,options) {
    if (options.stageReplay || !stats.sSkill.nativeCastId) return false;
...
function getAttackProjectileShots(stats) {
    return stats.sSkill.nativeCastId ? stats.projectileExtraShots : rollProjectileExtraShots(stats.projectileExtraShots);
}

function updateSkillGemCombat(stats) {
    const runtime=skillGemCombatRuntime;
    if (!runtime) return;
    if (game.combatHalted || game.playerHp<=0 || skillGemRuntimeChanged(game.activeSkill)) {
...
function skillGemRuntimeChanged(name) {
    return !!skillGemCombatRuntime.lastSkill && skillGemCombatRuntime.lastSkill!==name;
}

function initializeSkillGemFacing() {
    for (const enemy of game.enemies) if (!enemy.facingDirection) enemy.facingDirection=skillGemCasts.facing(enemy,game.gridPlayer);
}

function syncSkillGemChannel(runtime) {
    if (!runtime.channel) return;
    const c=runtime.channel.source,p=game.gridPlayer;
    const valid=game.activeSkill==='인과' && c.gx===p.gx && c.gy===p.gy && !hasPlayerChannelBreakingAilment();
    if (!valid || !combatChannelRuntime.id) cancelCombatChannel();
    else combatChannelRuntime.endAt=getCombatTime()+5000;
}
...
function applySkillGemCommand(command,stats) {
    ...
    if (command.type==='dot') return applySkillGemDot(command,targets);
    if (!targets.length) return;
    applySkillGemContact(command,targets,stats);
}
...
function applySkillGemContact(command,targets) {
    const stats=command.stats;
    const before=targets.map(e=>e.hp+(e.energyShield||0));
    performPlayerAttack({...stats,projectileExtraShots:0},{...command.attackOptions,stageReplay:true,skillName:command.name,
        sourceCell:command.sourceCell,targetEntries:targets.map(e=>({enemyId:e.id,mult:1})),stageRepeatOnce:true,
        restrictRandomTargets:true,skipSlamEcho:true,skipGridControl:true,
        hitDamageMultiplier:command.bonus?PROJECTILE_BONUS_SHOT_DAMAGE_PCT/100:1,damageTextGroupId:command.key});
    if (!command.ailment) return;
    targets.forEach((enemy,i)=>{
        if (enemy.hp<=0 || enemy.hp+(enemy.energyShield||0)>=before[i]) return;
        const ele=command.ailment==='poison'?'chaos':'phys';
        applyEnemyAilmentFromHit(enemy,{...stats,sSkill:{...stats.sSkill,ele}},stats.baseDmg,false,{primaryAilmentChance:1});
    });
}

function receiveSkillGemPlayerHit(damage,stats) {
    if (!skillGemCombatRuntime?.channel || hasPlayerChannelBreakingAilment()) return;
    const commands=skillGemCasts.receiveHit(skillGemCombatRuntime,{damage,alive:game.playerHp>0,
        source:game.gridPlayer,enemies:game.enemies,stats,now:getCombatTime()});
    for (const command of commands) applySkillGemCommand(command,stats);
}

function performPlayerAttack(pStats, attackOptions) {
    let options = attackOptions || {};
    if (shouldDeferNativeGemCast(pStats, options)) return;
    ...
    if (!isStageReplay) pStats = { ...pStats, projectileExtraShots: getAttackProjectileShots(pStats) };
    if (!isStageReplay && typeof isTalentPlayerAttackDisabled === 'function' && isTalentPlayerAttackDisabled()) return;
    ...
    if (!isStageReplay && typeof beginPassiveKarmaAttack === 'function') {
        passiveKarmaContext = beginPassiveKarmaAttack(targets[0].enemy, getCombatTime());
    }
    if (!isStageReplay) colosseumStrikeMul *= consumeLoneEvasionCounterMultiplier(pStats, getCombatTime());
    ...
    let previousFanaticismStacks = Math.max(0, Number(pStats.passiveFanaticismStacks) || 0);
    let currentFanaticismStacks = !isStageReplay && typeof recordPassiveFanaticSkillUse === 'function'
    ...
    if (!isStageReplay) {
        queuePlayerAttackSequence(pStats, pendingStages, {
            skillName, forcedCrit: isCrit, forcedElement: swingElement, stageCount,
            talentAttackMultiplier: talentAttackMul, attackDamageMultiplier: colosseumStrikeMul,
            talentMoonReturn, attackHitGuardCounts, passiveKarmaContext,
            channelContinuation: channelContinuationRequested, repeatChannelCast: !!options.repeatChannelCast
        });
        return;
    }
    ...
    let hasSkillConditionalFeedback = !!(pStats.sSkill.activeAilmentDamageMore
        || (pStats.sSkill.consumeAilmentDamageMore || []).length
        || pStats.sSkill.fullLifeDamageMorePct || pStats.sSkill.distanceDamageMoreCapPct
        || pStats.sSkill.crowdDamageMoreCapPct);
    ...
    let attackFeedbackGroup = options.damageTextGroupId ? String(options.damageTextGroupId) : '';
    let attackFeedbackKeys = new Set();
    function showAttackFeedback(enemy, cue) {
        if (!cue || !cue.key || !cue.text || attackFeedbackKeys.has(cue.key)) return;
        attackFeedbackKeys.add(cue.key);
        addBattleFx('statusText', {
            enemyId: enemy && enemy.id,
            text: cue.text,
            color: cue.color || '#f3d9a4',
            duration: 900,
            dedupeKey: attackFeedbackGroup ? `attack:${attackFeedbackGroup}:${cue.key}` : '',
            dedupeWindowMs: 1400
        });
    }
    function applyPierceOverkillCarry(sourceEnemy, carryDamage, hitElement, hitCrit, ailmentCarrySourceDamage) {
        ...
        for (let chainIdx = 0; chainIdx < chainLimit && remainingDamage > 0; chainIdx++) {
            ...
            addBattleFx('hit', {
                ...
                chain: true,
                stageKind: 'chainJump',
                chainFromEnemyId: chainSource && chainSource.id,
                skillName: skillName,
                damage: dealtToChain,
                duration: 320,
                element: hitElement,
    ...
    for (let hitIdx = 0; hitIdx < repeats; hitIdx++) {
        ...
        hitEntries.forEach((hit, targetIndex) => {
            ...
            transferPassiveChaosErosionToTarget(targetEnemy, pStats);
            ...
            if (pStats.passiveKeystoneFlags && pStats.passiveKeystoneFlags.projectileFormation && nextHitCount > 1) return;
            ...
            if (pStats.passiveKeystoneFlags && pStats.passiveKeystoneFlags.blackDistill
                ...
                enemyRes = (enemyRes + chaosRes) * 0.5;
            }
            ...
            let hitCrit = isCrit;
            let passiveGuaranteedCrit = false;
            if (pStats.passiveKeystoneFlags && (pStats.passiveKeystoneFlags.explosiveDistill
                || pStats.passiveKeystoneFlags.taintedWarhead)) hitCrit = false;
            if (pStats.passiveKeystoneFlags && pStats.passiveKeystoneFlags.fullEvasion
                && (pStats.sSkill.tags || []).includes('melee') && game.passiveFullEvasionCritReady) {
                hitCrit = true;
            ...
            if (pStats.passiveKeystoneFlags && (pStats.passiveKeystoneFlags.explosiveDistill
                || pStats.passiveKeystoneFlags.taintedWarhead)) {
                hitCrit = false;
                hitBaseDamage = pStats.baseDmg;
            }
            if (hitCrit && !guaranteedCrit && !passiveGuaranteedCrit && (targetEnemy.critResistPct || 0) > 0 && Math.random() * 100 < Math.max(0, Math.min(95, targetEnemy.critResistPct || 0))) {
                hitCrit = false;
                hitBaseDamage = pStats.baseDmg;
            }
            ...
            talentHitMul *= getPassiveKeystoneHitMultiplier(pStats, targetEnemy, hitIdx, targetIndex, skillName);
            ...
            } else if (pStats.passiveKeystoneFlags && pStats.passiveKeystoneFlags.blackDistill
                && ['fire', 'cold', 'light'].includes(hitElement)) {
                let convertedMultiplier = 0.5 + Math.max(0, Number(pStats.chaosDamageMultiplier) || 1) * 0.5;
                dmg = Math.floor(dmg * convertedMultiplier);
                ailmentSourceDamage = Math.floor(ailmentSourceDamage * convertedMultiplier);
            }
            ...
            dmg = Math.floor(dmg * Math.max(0, Number(talentConversion.mainPct) || 0)
                * (1 - (enemyRes / 100)) * wisdomMainMultiplier);
            damageBeforeMitigation = Math.floor(damageBeforeMitigation * wisdomMainMultiplier);
            ailmentDamageBeforeCritMitigation = Math.floor(ailmentDamageBeforeCritMitigation * wisdomMainMultiplier);
            // Resolve each element before combining the damage portions.
            if (hitElement === 'phys') {
            ...
            ['phys', 'fire', 'cold', 'light', 'chaos'].forEach(addEle => {
                let addPct = Math.max(0, Number(addedDamagePctByElement[addEle] || 0)
                    + Number(talentConversion.added && talentConversion.added[addEle] || 0));
                if (hitElement === 'phys' && addEle === 'fire') {
                    addPct += Math.max(0, Number(pStats.passiveCycleAddedFireFromPhysicalPct) || 0);
                }
                let wisdomAddedMultiplier = getPassiveWisdomLeapDamageMultiplier(pStats, addEle);
                let rawAdded = Math.floor(addedDamageSource * addPct / 100 * wisdomAddedMultiplier);
                ...
                if (addEle === 'phys') {
                    ...
                    mitigatedAdded = Math.floor(mitigatedAdded * (targetEnemy.physicalDamageTakenMul || 1) * Math.max(0.2, 1 - (targetEnemy.armorGuard || 0)));
                }
            ...
            ['phys', 'fire', 'cold', 'light', 'chaos'].forEach(addEle => {
                ...
                let wisdomFlatMultiplier = getPassiveWisdomLeapDamageMultiplier(pStats, addEle);
                let flatPortion = flatBase * wisdomFlatMultiplier
                    * (hitCrit ? Math.max(1, (Number(pStats.critDmg) || 100) / 100) * talentCritDmgMultiplier : 1)
                    * (rollPct / 100) * (hit.mult || 1) * repeatDamageMultiplier;
                let addRes = getEffectiveEnemyMitigation(addEle, zoneTier, targetEnemy, pStats) - (curseFx.resShred || 0);
                if (addEle === 'fire') addRes -= (curseFx.resFShred || 0);
                if (addEle === 'cold') addRes -= (curseFx.resCShred || 0);
                if (addEle === 'light') addRes -= (curseFx.resLShred || 0);
                ...
                if (addEle === 'phys') {
                    ...
                    mitigatedFlat = Math.floor(mitigatedFlat * (targetEnemy.physicalDamageTakenMul || 1) * Math.max(0.2, 1 - (targetEnemy.armorGuard || 0)));
                }
            ...
            let shockedIncreaseMul = getShockedEnemyHitDamageIncreaseMultiplier(targetEnemy, pStats);
            if (shockedIncreaseMul > 1) {
                dmg = Math.floor(dmg * shockedIncreaseMul);
            }
            ...
            let finalDamageMul = Number(pStats.finalDamageMultiplier);
            if (!Number.isFinite(finalDamageMul)) finalDamageMul = 1;
            finalDamageMul = Math.max(0, finalDamageMul);
            ...
            if (passiveKarmaContext && String(passiveKarmaContext.targetId) === String(targetEnemy.id)) {
                let karmaMul = Math.max(1, Number(passiveKarmaContext.multiplier) || 1);
                dmg = Math.floor(dmg * karmaMul);
                ailmentDamageBeforeCritMitigation = Math.floor(ailmentDamageBeforeCritMitigation * karmaMul);
            }
            if ((pStats.passiveOmniscientTrueDamagePct || 0) > 0) {
            ...
            if (targetEnemy && targetEnemy.id && pStats.uniqueCursedTakenAndRefresh) {
                ...
                if (refreshSec > 0 && game.enemyConditionDebuffs && Array.isArray(game.enemyConditionDebuffs[targetEnemy.id])) {
                    let nextExpire = getCombatTime() + Math.floor(refreshSec * 1000);
                    game.enemyConditionDebuffs[targetEnemy.id] = game.enemyConditionDebuffs[targetEnemy.id].map(deb => {
                        if (!deb) return deb;
                        if ((deb.expiresAt || 0) > getCombatTime()) deb.expiresAt = Math.max(deb.expiresAt || 0, nextExpire);
                        return deb;
                    });
                }
            }
            ...
            let directHitDistanceMul = getGridDirectHitDistanceMultiplier(game.gridPlayer, targetEnemy);
            dmg = Math.floor(dmg * directHitDistanceMul);
            damageBeforeMitigation = Math.floor(damageBeforeMitigation * directHitDistanceMul);
            ailmentDamageBeforeCritMitigation = Math.floor(ailmentDamageBeforeCritMitigation * directHitDistanceMul);
            targetEnemy.lastHitElement = hitElement;
            let moonShadowRatio = hitCrit && typeof getPreciseTalentRatio === 'function'
                ? 0.20 * getPreciseTalentRatio('hero3__assassin') : 0;
            let normalDamage = moonShadowRatio > 0 ? Math.floor(dmg * (1 - moonShadowRatio)) : dmg;
            let dealtToEnemy = applyDamageToEnemyResource(targetEnemy, normalDamage);
            if (dealtToEnemy > 0) {
                if (skillConditionalCues) skillConditionalCues.forEach(cue => showAttackFeedback(targetEnemy, cue));
                if (stageKind === 'slamAftershock') {
                    showAttackFeedback(targetEnemy, { key:'slam-aftershock', text:stageLabel || '여진', color:'#f0bd79' });
                }
                if (talentWasFull && (pStats.firstStrikeDamagePct || 0) > 0) {
            ...
            if (dmg > 0 && targetEnemy.hp > 0 && !targetEnemy.isBoss && (pStats.cullStrikePct || 0) > 0) {
                ...
                if (cullThr > 0 && (targetEnemy.hp / Math.max(1, targetEnemy.maxHp || targetEnemy.hp || 1)) <= cullThr) {
                    ...
                    showAttackFeedback(targetEnemy, { key:'cull', text:'처형', color:'#ff9b79' });
                }
            }
            ...
            if (dmg > 0 && targetEnemy.hp > 0 && typeof getTalentDotOccupancyDamage === 'function'
                && getTalentDotOccupancyDamage(targetEnemy, pStats) >= targetEnemy.hp) {
                dealtToEnemy += applyDamageToEnemyResource(targetEnemy, targetEnemy.hp);
                showAttackFeedback(targetEnemy, { key:'dot-execute', text:'지속 피해 처형', color:'#c88cff' });
            }
            ...
            if (dmg > 0 && targetEnemy.hp > 0 && typeof isTalentCardActive === 'function' && isTalentCardActive('hero1__inquisitor')) {
                ...
                if ((mk.cooldownUntil || 0) <= nowMk) {
                    ...
                    mk.accumulated = (mk.accumulated || 0) + dealtToEnemy;
            ...
            if (dmg > 0 && typeof isTalentCardActive === 'function' && isTalentCardActive('hero1__warlock')
                ...
                && game.enemyConditionDebuffs[targetEnemy.id].some(d => d && (d.expiresAt || 0) > getCombatTime())) {
                let activeCurses = game.enemyConditionDebuffs[targetEnemy.id];
                let consumedCurseIndex = activeCurses.findIndex(d => d && (d.expiresAt || 0) > getCombatTime());
                if (consumedCurseIndex >= 0) activeCurses.splice(consumedCurseIndex, 1);
            ...
            if (!options.skipSlamEcho && slamEchoPct > 0 && (pStats.sSkill.tags || []).includes('slam') && dmg > 0 && targetEnemy.hp > 0 && (slamEchoGuaranteed || passiveSlamEchoChance <= 0 || Math.random() < passiveSlamEchoChance)) {
                ...
                game.pendingSlamEchoHits.push({
                    at: getCombatTime() + slamEchoDelayMs,
                    enemyId: targetEnemy.id,
                    damage: Math.max(1, Math.floor(slamEchoBaseDamage * slamEchoPct)),
                    element: hitElement
                });
            }
            if ((pStats.damageScales || {}).talismanBossFinalDmgBonusPct && targetEnemy.hp > 0 && (targetEnemy.hp / Math.max(1, targetEnemy.maxHp || targetEnemy.hp)) <= 0.05) {
                targetEnemy.hp = 0;
                showAttackFeedback(targetEnemy, { key:'boss-finish', text:'마무리', color:'#f4c779' });
            }
            if (targetEnemy.hp > 0 && !targetEnemy.isBoss && !targetEnemy.isElite && !targetEnemy.elite
                && getCombatTalentCardLevel('hero2__assassin') > 0) {
                let butcherRow = game.talentButcherMarks && game.talentButcherMarks[targetEnemy.id];
                let butcherLifeRatio = targetEnemy.hp / Math.max(1, targetEnemy.maxHp || targetEnemy.hp);
                if (butcherRow && butcherRow.hits >= 4 && butcherLifeRatio <= 0.30) {
                    targetEnemy.hp = 0;
                    showAttackFeedback(targetEnemy, { key:'butcher-execute', text:'처형', color:'#ff9b79' });
                }
            }
            ...
            if (canApplyTalentExecuteThreshold(targetEnemy, pStats.sSkill.executeThreshold) && targetEnemy.hp > 0
                && (targetEnemy.hp / Math.max(1, targetEnemy.maxHp || targetEnemy.hp)) < pStats.sSkill.executeThreshold) {
                targetEnemy.hp = 0;
            ...
            if (game.ascendClass === 'gladiator' && hasKeystone('g6') && targetEnemy.hp > 0) {
                ...
                if ((targetEnemy.hp / Math.max(1, targetEnemy.maxHp || targetEnemy.hp)) < executeThreshold) {
                    targetEnemy.hp = 0;
                    showAttackFeedback(targetEnemy, { key:'gladiator-execute', text:'처형', color:'#ff9b79' });
                }
            }
            ...
            if (pStats.uniqueQueenBeeSummon && Math.random() < Math.max(0, Math.min(1, (pStats.uniqueQueenBeeSummon.chance || 0) / 100))) {
                ...
                bees.push({ expiresAt: getCombatTime() + Math.max(1000, Math.floor((pStats.uniqueQueenBeeSummon.attacks || 3) * 1000)), nextAt: getCombatTime() + 1000, attacksLeft: Math.max(1, Math.floor(pStats.uniqueQueenBeeSummon.attacks || 3)), hitPct: Math.max(1, Number(pStats.uniqueQueenBeeSummon.hitPct || 125)) });
            ...
            if (pStats.uniqueDragonVeinGuard && Math.random() < Math.max(0, Math.min(1, (pStats.uniqueDragonVeinGuard.chance || 0) / 100))) {
                ...
                game.playerUniqueGuard = { amount: guardAmt, expiresAt: getCombatTime() + Math.max(500, Math.floor((pStats.uniqueDragonVeinGuard.duration || 2) * 1000)) };
            }
    ...
    const instantLeechRecovered = applyPlayerHitLeech(pStats, totalLeechableDamage, totalChaosDamage);
...
function handlePlayerDefeat(zone, pStats, message, options) {
    ...
    if (zone && zone.id === OUTSIDE_CHAOS_ZONE_ID) {
        ...
        game.killsInZone = 0;
    } else if (zone && zone.type === 'beyondBoundary') {
        let returnZoneId = abandonBeyondBoundaryRun(game);
        addLog(message || '경계 너머 도전에 실패했습니다. 진행 중이던 단계가 종료됩니다.', 'death', { noToast: !!opts.noToast });
        game.currentZoneId = returnZoneId !== null && returnZoneId !== undefined ? returnZoneId : getDefeatRecoveryZoneId();
        game.killsInZone = 0;
        game.enemies = [];
        game.encounterPlan = [];
        game.encounterIndex = 0;
    ...
    if (existing) {
        ...
        if (damageAilment) {
            ...
            if (incomingScore >= existingScore) {
                ...
                existing.sourceEnemyId = opts.sourceEnemyId === undefined ? null : opts.sourceEnemyId;
                existing.sourceEnemyName = opts.sourceEnemyName || '';
            }
        }
    } else {
        ...
        if (damageAilment) {
            ...
            row.sourceEnemyId = opts.sourceEnemyId === undefined ? null : opts.sourceEnemyId;
            row.sourceEnemyName = opts.sourceEnemyName || '';
        }
        game.playerAilments.push(row);
    }
    if (typeof recordPassiveCycleAilmentStart === 'function') recordPassiveCycleAilmentStart(type, pStats.cycle, getCombatTime());
}

function recordPlayerAilmentIncomingDamage(ailment, element, amount) {
    let label = getAilmentDisplayLabel(ailment && ailment.type);
    let hasMonster = ailment && ailment.sourceEnemyId !== null && ailment.sourceEnemyId !== undefined;
    recordIncomingDamage(element, amount, (ailment && ailment.sourceEnemyName) || label, {
        sourceType: hasMonster ? 'monster' : 'ailment',
        sourceId: hasMonster ? ailment.sourceEnemyId : null,
        sourceName: (ailment && ailment.sourceEnemyName) || label,
        ailmentType: ailment && ailment.type
    });
}
...
function tickAilments(pStats, dt) {
    ...
    let next = [];
    game.playerAilments.forEach(ail => {
        let previousTime = Math.max(0, Number(ail.time) || 0);
        ail.time = Math.max(0, (ail.time || 0) - dt);
        ...
        if (ail.type === 'ignite') {
            ...
            if (poison > 0) {
                ...
                if (pStats.poisonToHeal) {
                    ...
                    recordPlayerAilmentIncomingDamage(ail, 'chaos', poison);
                }
            }
        } else if (ail.type === 'bleed') {
            let bleed = getPlayerDamageAilmentDps(ail, pStats);
            if (bleed > 0) {
                let effectiveBleedDr = (pStats.dr || 0);
        ...
        if (ail.time > 0) next.push(ail);
        else if (previousTime > 0 && typeof recordPassiveCycleAilmentEnd === 'function') {
            recordPassiveCycleAilmentEnd(ail.type, pStats && pStats.cycle, getCombatTime());
        }
    });
...
function getEnemyPreferredGridTarget(enemy) {
    ...
    (game.summons || []).forEach(summon => {
        ...
        let distance = getGridUnitDistance(enemy, summon);
...
function isEnemyInGridAttackRange(enemy, target) {
    ...
    return getGridUnitDistance(enemy, target) <= range;
}
...
function getClosestLivingSummonToPlayer() {
    ...
    return (game.summons || []).filter(summon => summon && !summon.isGhost && summon.alive && summon.hp > 0).sort((a, b) => {
        let aDist = hasGridCell(player) && hasGridCell(a) ? getGridUnitDistance(player, a) : Infinity;
        let bDist = hasGridCell(player) && hasGridCell(b) ? getGridUnitDistance(player, b) : Infinity;
...
function applyMonsterDamageToSummon(summon, rawDamage, enemy, pStats) {
    ...
    if (resolveEntropyEvasion(summon, evade, getCombatTime())) return 0;
    ...
    if (pStats.uniqueSummonDeathDamageBuff) {
        ...
        game.summonDeathDamageBuffExpiresAt = getCombatTime() + Math.max(1, Number(config.duration || 4)) * 1000;
    }
    return damage;
}

/** Apply each summon hit once; return whether the player's defense pipeline must also run. */
function dispatchMonsterHitTargets(targets, damage, enemy, pStats) {
    targets.filter(unit => unit !== game.gridPlayer)
        .forEach(summon => applyMonsterDamageToSummon(summon, damage, enemy, pStats));
    return targets.includes(game.gridPlayer);
}
...
function performMonsterAttacks(pStats) {
    ...
    pendingEnemyCombatAttacks.forEach(row => {
        if (row && row.at <= getCombatTime() && row.source && !attackSources.some(enemy => enemy && enemy.id === row.enemyId)) attackSources.push(row.source);
    });
    for (let enemy of attackSources) {
        let pendingAttack = takePendingEnemyCombatAttack(enemy.id, getCombatTime());
        ...
        if (!pendingAttack && !enemy.patternArea && !isEnemyInGridAttackRange(enemy, attackTarget)) {
        ...
        while (pendingAttack || enemy.attackTimer >= 1) {
            ...
            if (zone.type === 'outsideChaos') {
                ...
                if (getCombatTime() >= enemy.nextCurseAt) {
                    enemy.nextCurseAt = getCombatTime() + 6500;
                    let curseType = rndChoice(['ignite','chill','shock','poison','bleed']);
                    applyPlayerAilment(curseType, 4, 0.18, pStats, 0, {
            ...
            if (attackTarget === game.gridPlayer) {
                if (skillGemCombatRuntime) enemy.facingDirection=skillGemCasts.facing(enemy,game.gridPlayer);
                addBattleFx('enemyAttack', { enemyId: enemy.id, duration: 220 });
            }
            let dmg = getMonsterBaseHitDamage(zone, seasonDepth, tierPressure, benchmarkProfile);
            ...
            if (!dispatchMonsterHitTargets(impactTargets, dmg, enemy, pStats)) break;
            if (getCombatTime() < Math.max(0, Number(game.realmInvulnerableBarrierUntil) || 0)) {
            ...
            damageBreakdown = damageBreakdown.map(row => {
                ...
                else if (row.ele === 'chaos') {
                    less = (pStats.chaosTakenDamageReducePct || 0) + (pStats.uniqueChaosTakenDamageReducePct || 0);
                }
            ...
            while (allFlatLess > 0 && damageBreakdown.length > 0) {
                let targetIndex = damageBreakdown.reduce((largest, row, index) =>
                    (row.amount || 0) > (damageBreakdown[largest].amount || 0) ? index : largest, 0);
            ...
            if (pStats.passiveAshura && typeof applyPassiveAshuraDamageBreakdown === 'function') {
                damageBreakdown = applyPassiveAshuraDamageBreakdown(damageBreakdown, game.playerAilments, pStats.cycle);
            }
            ...
            let directHitDistanceMul = getGridDirectHitDistanceMultiplier(enemy, game.gridPlayer);
            scaleBreakdown(directHitDistanceMul);
            dmg = Math.max(1, sumBreakdown());
            ...
            if (!wasBlocked && deflectRoll < Math.max(0, Math.min(75, pStats.deflectChance || 0))) {
                ...
                if (pStats.uniqueDeflectStealth) {
                    ...
                    game.shadowStealthExpiresAt = getCombatTime() + Math.max(1, Number(cfg.duration || 3)) * 1000;
                }
                deflected = true;
            ...
            if (enemyAilmentChance > 0 && ailRoll < enemyAilmentChance) {
                let ail = enemy.ele === 'fire' ? 'ignite' : enemy.ele === 'cold' ? 'chill'
                    : enemy.ele === 'light' ? 'shock' : enemy.ele === 'phys' ? 'bleed' : 'poison';
                ...
                let ailmentLabel = ail === 'ignite' ? '점화' : ail === 'chill' ? '냉각'
                    : ail === 'shock' ? '감전' : ail === 'bleed' ? '출혈' : '중독';
                if (game.settings.showCombatLog) addLog(`☣️ 상태이상: ${ailmentLabel} (${enemy.isBoss ? 5 : 3}초)`, 'attack-monster', { element:enemy.ele });
            }

            grantWarriorRageOnHit(getCombatTime());
            if (typeof getPreciseTalentRatio === 'function' && getPreciseTalentLevel('hero5__guardian')) {
                let missingLife = Math.max(0, getPlayerRecoveryHpCap(pStats) - Math.max(0, game.playerHp || 0));
                    ...
                    aliveGuards.forEach((guard, idx) => {
                        ...
                        if (guard.hp <= 0) {
                            ...
                            if (pStats.uniqueSummonDeathDamageBuff) {
                                ...
                                game.summonDeathDamageBuffExpiresAt = getCombatTime() + Math.max(1, Number(cfg.duration || 4)) * 1000;
                            }
                        }
                    });
            ...
            let playerResourceBeforeHit = Math.max(0, Number(game.playerHp) || 0) + game.playerEnergyShield;
            ...
            triggerUniqueEnergyShieldBreakRecharge(pStats, beforeEsForCr8, getCombatTime());
            ...
            if (remaining > 0 && game.playerUniqueGuard && getCombatTime() < (game.playerUniqueGuard.expiresAt || 0) && (game.playerUniqueGuard.amount || 0) > 0) {
            ...
            if (typeof recordPassiveKarmaLoss === 'function') {
                let playerResourceAfterHit = Math.max(0, Number(game.playerHp) || 0)
...
function calculateTrialTrapDamage(zone, pStats, trapElement) {
    ...
    let physicalMitigation = (Number(pStats.dr) || 0) + getArmorPhysicalReductionPct(pStats.armor, scaledDamage);
    let mitigation = trapElement === 'phys' ? physicalMitigation : (Number(pStats[resistanceKey]) || 0);
...
function buildTrialHazardCells(zone, origin, sequence) {
    ...
    let push = (gx, gy) => {
        ...
        if (!isGridCellInBounds(gx, gy) || seen.has(key)) return;
        seen.add(key);
        cells.push({ gx, gy });
    };
    let horizontal = sequence % 2 === 0;
    if (profile.pattern === 'line' || profile.pattern === 'doubleLine') {
        let adjacent = horizontal
            ? (origin.gy < rows - 1 ? origin.gy + 1 : origin.gy - 1)
            : (origin.gx < columns - 1 ? origin.gx + 1 : origin.gx - 1);
        let length = horizontal ? columns : rows;
        for (let index = 0; index < length; index++) {
            if (horizontal) {
                push(index, origin.gy);
                if (profile.pattern === 'doubleLine') push(index, adjacent);
            } else {
                push(origin.gx, index);
                if (profile.pattern === 'doubleLine') push(adjacent, index);
            }
        }
    } else if (profile.pattern === 'cross') {
        for (let gx = 0; gx < columns; gx++) push(gx, origin.gy);
        for (let gy = 0; gy < rows; gy++) push(origin.gx, gy);
    } else {
        for (let dx = -1; dx <= 1; dx++) {
            for (let dy = -1; dy <= 1; dy++) {
                if (profile.pattern === 'pool' && Math.abs(dx) + Math.abs(dy) > 1) continue;
                push(origin.gx + dx, origin.gy + dy);
            }
        }
    }
    return cells;
...
function getTrialHazardAvoidThreshold(profile) {
    let mode = game.settings && game.settings.combatPositionMode;
    let baseThreshold = mode === 'keepRange' ? 0.1 : (mode === 'pressure' ? 0.32 : 0.18);
    return baseThreshold * profile.avoidThresholdMul;
}
...
function createTrialHazard(zone, pStats, now) {
    let index = Math.max(0, Math.floor(game.trialHazardIndex || 0));
    let elements = Array.isArray(zone.trapElements) && zone.trapElements.length > 0 ? zone.trapElements : ['phys'];
    let element = elements[index % elements.length];
    let profile = getTrialHazardProfile(zone);
    let cells = buildTrialHazardCells(zone, game.gridPlayer, index);
    game.trialHazardIndex = index + 1;
    let color = { phys: '#ffd36b', fire: '#ff754f', cold: '#86dcff', light: '#e5d35b', chaos: '#c47cff' }[element] || '#ffd36b';
    addBattleFx('trialTrapWarning', { targetCells: cells, color, element, duration: profile.warningMs });
    let hazard = {
        cells,
        element,
        color,
        impactAt: now + profile.warningMs,
        intervalMs: profile.intervalMs,
        avoidThreshold: getTrialHazardAvoidThreshold(profile),
        shouldAvoid: false
    };
    hazard.shouldAvoid = shouldAvoidTrialHazard(zone, pStats, hazard);
    return hazard;
}

function isPlayerInsideCombatHazard(hazard) {
    if (!hazard || !hasGridCell(game.gridPlayer)) return false;
    let playerKey = gridCellKey(game.gridPlayer.gx, game.gridPlayer.gy);
    return hazard.cells.some(cell => gridCellKey(cell.gx, cell.gy) === playerKey);
}

function isTrialHazardPositionHeld() {
    let hazard = trialHazardRuntime.active;
    return !!(hazard && hazard.shouldAvoid && getCombatTime() < hazard.impactAt && !isPlayerInsideCombatHazard(hazard));
}

/** Advance one shared escape timer for all warning cells; never move twice in a combat tick. */
function updateCombatHazardEscape(hazard, pStats, now, reason = '함정 회피') {
    if (!hazard.shouldAvoid || !isPlayerInsideCombatHazard(hazard)) return false;
    if (hasPlayerChannelBreakingAilment()) return true;
    let moveSpeed = Number.isFinite(pStats.moveSpeed) && pStats.moveSpeed > 0 ? pStats.moveSpeed : 100;
    let interval = COMBAT_GRID_CONFIG.playerMoveIntervalSec * (100 / moveSpeed);
    let result = advanceGridHazardEscape(game.gridPlayer, hazard.cells, 0.1, interval);
    if (result.blocked || !result.moved) return true;
    cancelCombatChannel(reason);
    let attackDelayMs = getTacticalMoveAttackDelayMs(moveSpeed);
    combatTacticsRuntime.attackDelayUntil = Math.max(combatTacticsRuntime.attackDelayUntil, now + attackDelayMs);
    addBattleFx('playerMobility', { fromCell: result.from, toCell: result.to, duration: 240 });
    return true;
}
...
function updateCombatHazardEvasion(pStats) {
    ...
    let holdPosition = getBossWarningCells(game, pendingEnemyCombatAttacks).length > 0 || isTrialHazardPositionHeld();
    let cells = [];
    if (trialHazardRuntime.active && trialHazardRuntime.active.shouldAvoid) cells.push(...trialHazardRuntime.active.cells);
    let avoiding = updateCombatHazardEscape({ cells, shouldAvoid: true }, pStats, getCombatTime(), '위험 범위 회피');
    let frozen = game.playerAilments.some(ail => ail.type === 'freeze' && ail.time > 0);
...
function dealTrialTrapDamage(zone, pStats, hazard, trapDamage) {
    ...
    triggerUniqueEnergyShieldBreakRecharge(pStats, energyShieldBeforeTrap, getCombatTime());
    if (remaining > 0) remaining = absorbDamageWithTalentStoneShield(remaining);
    if (remaining > 0) remaining = absorbDamageWithRealmDeathWard(remaining, pStats);
...
function resolveTrialHazardImpact(zone, pStats, hazard, now) {
    let playerHit = isPlayerInsideCombatHazard(hazard);
    let barrierActive = playerHit && now < Math.max(0, Number(game.realmInvulnerableBarrierUntil) || 0);
    addBattleFx('trialTrap', {
    ...
    if (!playerHit) {
        ...
        dealTrialTrapDamage(zone, pStats, hazard, calculateTrialTrapDamage(zone, pStats, hazard.element));
    }
...
function applyTrialTrapTick(pStats, deferEscape) {
    ...
    if (!(game.enemies || []).some(enemy => enemy && enemy.hp > 0)) {
        ...
        trialHazardRuntime.nextAt = Math.max(trialHazardRuntime.nextAt, now + 500);
        return false;
    }
    if (!trialHazardRuntime.nextAt) trialHazardRuntime.nextAt = now + getTrialHazardProfile(zone).initialDelayMs;
    if (!trialHazardRuntime.active && now >= trialHazardRuntime.nextAt) {
        trialHazardRuntime.active = createTrialHazard(zone, pStats, now);
    }
    let hazard = trialHazardRuntime.active;
    if (!hazard) return false;
    if (!hazard.shouldAvoid) hazard.shouldAvoid = shouldAvoidTrialHazard(zone, pStats, hazard);
    if (now >= hazard.impactAt) {
        resolveTrialHazardImpact(zone, pStats, hazard, now);
        return false;
    }
    return !deferEscape && updateCombatHazardEscape(hazard, pStats, now);
}

function applyLoopHeroSelection(classId, previousClassId) {
    game.pendingLoopHeroSelection = false;
    saveGame({ skipCloudSync: true });
    if (typeof requestImmediateCloudSave === 'function') requestImmediateCloudSave('루프 직업 선택 완료');
    startMoving(true);
    dispatchRuntimeEvent('loop-hero-selection-completed', {
        classId,
        changed: classId !== previousClassId
    });
}

function requestLoopHeroSelection(options = {}) {
    let previousClassId = game.selectedClassId || 'archer';
    let detail = {
        handled: false,
        options,
        select: classId => applyLoopHeroSelection(classId, previousClassId)
    };
 ...
 * 루프 10부터는 정산을 확인한 뒤 다음 루프로 가거나 심화 등반을 계속한다.
 */
