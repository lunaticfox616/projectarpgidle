...
function refreshItemIdCounter() {
    ...
    // 최근 획득함도 id 공간을 점유한다 — 누락하면 재로드 후 id가 재사용되어 배치/제작이 다른 아이템을 가리킨다.
    let recentIds = (game.recentGrowthDrops || []).filter(Boolean).map(item => item.id || 0);
    itemIdCounter = Math.max(0, ...(game.inventory || []).map(item => item.id || 0), ...Object.values(game.equipment || {}).filter(Boolean).map(item => item.id || 0), ...altarIds, ...recentIds);
}
