...
function updateCombatUI(pStats) {
    ...
    if (hpText) hpText.innerText = `${Math.max(0, Math.floor(game.playerHp))} / ${pStats.maxHp}${(pStats.energyShield || 0) > 0 ? ` · ES ${Math.floor(game.playerEnergyShield || 0)}/${Math.floor(pStats.energyShield)}` : ''}`;
