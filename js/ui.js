...
function showGemTooltip(event, type, name) {
    ...
    if (type === 'support') {
        ...
        if ((skill.fireResOvercapMulPerPct || 0) > 0) {
            ...
            let currentFireRes = stats && Number.isFinite(stats.resF) ? stats.resF : null;
            let maxFireRes = stats && Number.isFinite(stats.maxResF) ? stats.maxResF : null;
            let appliedOvercap = currentFireRes !== null ? Math.max(0, currentFireRes - 75) : null;
            let currentText = currentFireRes !== null
                ? ` · 현재 적용분 ${appliedOvercap.toFixed(1)}% (현재 화염 저항 ${Math.floor(currentFireRes)}%${maxFireRes !== null ? `/${Math.floor(maxFireRes)}%` : ''}, 최대치 초과분 제외)`
                : ' · 최대치를 초과한 화염 저항은 제외';
            html += `<div class="tooltip-line">초과 화염 저항 계수: 75% 초과 현재 화염 저항 1%당 배율 +${Number(skill.fireResOvercapMulPerPct || 0).toFixed(2)}배${currentText}</div>`;
        }
