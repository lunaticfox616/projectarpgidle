...
assert.strictEqual(list.filter(row=>row.family!=='hitSpark').length, 1, 'one actual cleave stage must draw one arc across multiple victims');
queue({ ...hit, id: 3, repeatIndex: 1 });
assert.strictEqual(list.filter(row=>row.family!=='hitSpark').length, 1, 'native bridge groups repeats within the same cast stage');
const [first] = list.filter(row=>row.family!=='hitSpark');
assert.strictEqual(first.startAt,1000,'native timing must not invent an extra 65ms delay');
assert.strictEqual(list.filter(row=>row.family==='hitSpark').length,3,'each confirmed victim retains its native half-size contact');
...
read('delete battleAssets.images.skillFxWorldTree');
runtime.drawSkillGemVfxLayer(ctx, 1040);
assert(!calls.some(call => call.key === 'stroke'), 'missing supplied art must not resurrect a different procedural skill');
...
assert.strictEqual(list[0].toX,58,'the left-side target is retained for native directional placement');
list.length = 0;
queue({ ...hit, id: 5, damageTextGroupId: undefined });
queue({ ...hit, id: 6, damageTextGroupId: undefined });
assert.strictEqual(list.filter(row=>row.family!=='hitSpark').length, 1, 'native bridge groups unidentified hits by event time');
for (let id = 10; id < 100; id++) queue({ ...hit, id, damageTextGroupId: `fast:${id}` });
assert(list.length <= read('BATTLE_SKILL_EFFECT_CAP'), 'rapid attacks remain within the shared visual-event budget');
...
assert.strictEqual(actualList.filter(row=>row.family!=='hitSpark').length, 1, 'native bridge displays one stage for the cast while retaining both damage hits');
...
assert.strictEqual(run('battleVisualState.skillEffects.filter(row=>row.family!=="hitSpark").length'), 1, 'a first-hit kill must show only its confirmed sweep');
