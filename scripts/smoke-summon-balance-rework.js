...
Object.entries(expectedBaseHp).forEach(([name, hp]) => {
    assert.strictEqual(runtime.getSummonProfile(name).baseHp, hp, `${name}의 baseHp가 기존 대비 50% 수준으로 낮아져야 한다`);
});

// ── 2. 최소 피해 편차(dmgRollMinPct)가 프로필에 30~50 사이로 존재해야 한다 ──
Object.keys(expectedBaseHp).forEach(name => {
    let pct = runtime.getSummonProfile(name).dmgRollMinPct;
...
const rollHalf = runtime.getSummonHitDamageInfo(hitProfile, pStats, null, { rollOverridePct: 50, forceCrit: false });
const rollFull = runtime.getSummonHitDamageInfo(hitProfile, pStats, null, { rollOverridePct: 100, forceCrit: false });
...
const burstMin = runtime.getSummonHitDamageInfo(burstProfile, pStats, null, { rollOverridePct: 30, forceCrit: false });
...
const step9to10 = runtime.getAttackSummonGrowthSteps(10) - runtime.getAttackSummonGrowthSteps(9);
