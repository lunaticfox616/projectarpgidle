...
const weaponOverlay = context.document.getElementById('weapon-slot-overlay');
...
vm.runInContext(`
  ...
  game.inventory = [{ id: 100, name: '빈 슬롯용 무기', slot: '무기' }];
  game.equipment = {
    무기: { id: 200, name: '기존 주 무기', slot: '무기' },
    방패: null,
    장갑1: null,
    장갑2: null,
    반지1: null,
...
assert.strictEqual(vm.runInContext('equipItemById(100)', context), true, 'weapon equip request must use an empty eligible slot');
assert.strictEqual(vm.runInContext('game.equipment.방패.id', context), 100, 'dual training must auto-equip into an empty shield slot');
assert(!weaponOverlay.classList.contains('active'), 'empty eligible slot must not open the selector');

vm.runInContext(`
  game.inventory = [{ id: 105, name: '빈 주 무기용 무기', slot: '무기' }];
  game.equipment.무기 = null;
`, context);
vm.runInContext('equipItemById(105)', context);
assert.strictEqual(vm.runInContext('game.equipment.무기.id', context), 105, 'dual training must auto-equip into an empty main weapon slot');
assert(!weaponOverlay.classList.contains('active'), 'empty main weapon slot must not open the selector');
...
assert(weaponOverlay.classList.contains('active'), 'dual training weapon equip must open the slot selector');
...
vm.runInContext(`
  ...
  game.inventory = [{ id: 104, name: '일반 워리어 무기', slot: '무기' }];
  game.equipment.무기 = { id: 204, name: '기존 일반 무기', slot: '무기' };
  game.equipment.방패 = null;
`, context);
vm.runInContext('equipItemById(104)', context);
assert.strictEqual(vm.runInContext('game.equipment.무기.id', context), 104, 'without dual training a weapon must use only the main weapon slot');
assert.strictEqual(vm.runInContext('game.equipment.방패', context), null, 'without dual training the shield slot must remain unavailable to weapons');
