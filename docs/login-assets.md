...
`assets/ui/gameplay-intro.webm`은 실제 전투 캔버스를 30fps, VP8로 녹화하고 몬스터 처치 약 0.65초 전부터 2초로 자른 무음 영상이다. `gameplay-intro.webp`은 같은 녹화의 레벨업 프레임이다.
별도의 브라우저 컨텍스트에서 새 전사를 시작하고 기존 `grantLoopStarterGemOnFirstKill`/`changeSkill`로 연속 베기를 먼저 장착한 예시다.
...
첫 클릭에만 미디어 경로를 지정하며 닫기·Escape 시 정지하고 처음으로 되돌린다. 동작 줄이기 설정에서는 수동 재생한다.
